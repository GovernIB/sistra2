...
public class UtilPDF {
	 ...
	 * @param pdfIn
	 *            InputStreams de los pdfs a concatenar
	 */
	public static void concatenarPdf(final OutputStream pdfOut, final InputStream[] pdfIn) throws Exception {
		if (pdfIn.length < 2) {
	...
	public static void extractPages(final OutputStream pdfOut, final InputStream pdfIn, final int[] pages)
		...
		if (pages.length <= 0) {
			throw new Exception("Debe eliminar al menos una pagina");
		}

		final List pagesToKeep = new ArrayList();
	 ...
	 * 3: set permission writer.setEncryption(null, null, permission,
	 * PdfWriter.STRENGTH40BITS); // step 4: we open the document document.open(); }
	 * // step 4: we add content PdfImportedPage page; for (int i = 0; i < n; ) {
	 * ++i; page = writer.getImportedPage(reader, i); writer.addPage(page); }
	 ...
	 *            Pdf entrada
	 * @param objects
	 *            Objetos a realizar el stamp
	 * @throws Exception
	 */
	public static void stamp(final OutputStream pdfOut, final InputStream pdfIn, final ObjectStamp[] objects)
			throws Exception {

		final PdfReader reader = new PdfReader(pdfIn);

		final int permission = reader.getPermissions();
		final int nPages = reader.getNumberOfPages();
		final PdfStamper stamp = new PdfStamper(reader, pdfOut);
			...
			for (int j = 0; j < objects.length; j++) {
				if (objects[j].getPage() == 0 || objects[j].getPage() == i) {
					...
					if (objects[j] instanceof ImageStamp) {
						final ImageStamp imgStamp = (ImageStamp) objects[j];
						final Image img = Image.getInstance(imgStamp.getImagen());
					...
					if (objects[j] instanceof BarcodeStamp) {
						...
						Image img = null;
						final BarcodeStamp barcodeStamp = (BarcodeStamp) objects[j];

						switch (barcodeStamp.getTipo()) {
						case BarcodeStamp.BARCODE_128:
							final Barcode128 code128 = new Barcode128();
							final BaseFont bf = BaseFont.createFont(barcodeStamp.getFontName(), BaseFont.CP1252,
						...
						if (barcodeStamp.getXScale() != null && barcodeStamp.getYScale() != null) {
							img.scalePercent(barcodeStamp.getXScale().floatValue(),
									barcodeStamp.getYScale().floatValue());
						}
					...
					if (objects[j] instanceof TextoStamp) {
						final BaseFont bf = BaseFont.createFont(((TextoStamp) objects[j]).getFontName(),
					...
					if (objects[j] instanceof NumeroPaginaStamp) {
						...
						content.setTextMatrix(30, 30);
						final String texto = i + "/" + nPages;
						content.showTextAligned(Element.ALIGN_CENTER, texto, objects[j].getX(), objects[j].getY(),
								objects[j].getRotation());
						content.endText();
					}
					...
					if (objects[j] instanceof SelloEntradaStamp) {
						final int width = 202;
						final int height = 92;
						final float posX = objects[j].getX();
						final float posY = objects[j].getY();
						...
						content.setTextMatrix(30, 30);
						final String textoReservado = "Registre d'entrada";
						content.showTextAligned(Element.ALIGN_LEFT, textoReservado,
								posX - (width / 2) - (textoReservado.length() * 2), posY - 8, 0);
						content.endText();
					}
	 ...
	 * @throws Exception
	 */
	public static int getNumberOfPages(final InputStream pdfIn) throws Exception {
		final PdfReader reader = new PdfReader(pdfIn);
	 ...
	 * @param pdfIn
	 *            InputStreams del pdf sobre el que se deben aplicar los permisos
	 */
	public static void establecerSoloImpresion(final OutputStream pdfOut, final InputStream pdfIn) throws Exception {
		final PdfReader reader = new PdfReader(pdfIn);
	 ...
	 * @param pdfIn
	 *            InputStreams del pdf sobre el que se deben quitar los permisos
	 */
	public static void eliminarPermisos(final OutputStream pdfOut, final InputStream pdfIn) throws Exception {
		final PdfReader reader = new PdfReader(pdfIn);
		final PdfStamper stamper = new PdfStamper(reader, pdfOut);
		stamper.close();
		reader.close();
	}
	 ...
	 * @throws Exception
	 */
	public static boolean isPdfA(final InputStream pdfIn) throws Exception {
		final PdfReader reader = new PdfReader(pdfIn);
		final byte metaBytes[] = reader.getMetadata();
		if (metaBytes == null) {
			return false;
		}
		final String xml = new String(metaBytes, "UTF-8");
