...
public class FormateadorGenerico implements FormateadorPdfFormulario {
	...
	/**
	 * Propiedad que se pasa por plantilla, indica como mostrar la visualización de
	 * listado de valores.
	 **/
	public static final String LISTADO_VISUALIZACION = "listado.visualizacion";
	/**
	 * Propiedad que se pasa por plantilla, indica si los mensajes de aviso se deben
	 * mostrar tambien el pdf.
	 **/
	...
	public byte[] formatear(final byte[] ixml, final byte[] plantilla, final String idioma,
		...
		final PDFDocument documento = new PDFDocument(defFormInterno.getTitulo());
		if (urlImagen != null) {
		...
		for (final RPaginaFormulario pagina : defFormInterno.getPaginas()) {
			...
			for (final RLineaComponentes linea : pagina.getLineas()) {
				for (final RComponente componente : linea.getComponentes()) {
					...
					// Si no se ha creado una sección y el componente no es de tipo sección
	...
	private void inicializarValores(final byte[] plantilla) {
		...
		if (plantilla != null) {
			try {
				...
				final Properties prop = SerializationUtils.deserialize(plantilla);
				...
				if (prop != null && prop.containsKey(LISTADO_VISUALIZACION)) {
					listadoVisualizacion = (TipoVisualizacionListado) prop.get(LISTADO_VISUALIZACION);
				}

				if (prop != null && prop.containsKey(AVISO_MOSTRAR)) {
					mostrarAviso = Boolean.valueOf(prop.get(AVISO_MOSTRAR).toString());
				}

			} catch (final Exception e) {
				throw new FormateadorException("Error obteniendo la url de propiedades que contiene la imagen", e);
			}
		}
	}
	...
	private Propiedad getPropiedad(final String etiqueta, final ValorCampoIndexado valor) {
		if (listadoVisualizacion == TipoVisualizacionListado.SOLO_CODIGO) {
			...
			return new Propiedad(etiqueta, getValorCampoIndexado(valor.getValor()));
		}
	}
