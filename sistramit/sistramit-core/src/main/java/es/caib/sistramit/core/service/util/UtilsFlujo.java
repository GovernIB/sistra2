...
public final class UtilsFlujo {
    ...
    /** Map para control limite tramitación. */
    private static final Map<String, Date> LIMITES_TRAMITACION = new HashMap<String, Date>();

    /** Map para control limite tramitación: excepciones auditadas. */
    private static final Map<String, Date> LIMITES_TRAMITACION_EXC = new HashMap<String, Date>();
     ...
     * @param pFormulariosCompletadosPaso Formularios anteriores completados en
     *                                    el paso actual
     ...
     * Convierte Java a JSON.
     *
     * @param jsonBytes Objeto en bytes
     * @param clase     Clase
     * @return JSON
     */
     ...
     *
     * @param cadena            cadena
     * @param confEntidad       Configuración entidad
     * @param identificadorArea Id área
     * @return
     */
    ...
    public static void controlLimitacionTramitacion(final DefinicionTramiteSTG defTram, FlujoTramiteDao flujoTramiteDao) {
        ...
        if (controlAcceso.isLimitarTramitacion()) {
            ...
            final String tramiteKey = idTramite + "-" + version;

            // Verifica si ya se ha llegado a limite en minuto actual
            Long total;
            final Date minutoActual = getMinutoActual();
            final Date ultimoLimite = LIMITES_TRAMITACION.get(tramiteKey);
            if (ultimoLimite != null && minutoActual.compareTo(ultimoLimite) == 0) {
                // Indicamos que ha llegado al total
                total = limitNumero;
            } else {
                // Cuentra tramites en minuto actual
                total = flujoTramiteDao.contadorLimiteTramitacion(idTramite, version, minutoActual);
            }

            // Si ha llegado al total, cacheamos resultado y lanzamos excepcion
            if (total.longValue() >= limitNumero) {
                // Cacheamos
                LIMITES_TRAMITACION.put(tramiteKey, minutoActual);
                // Auditamos excepcion solo 1 por minuto
                Date ultimaExcepcion = LIMITES_TRAMITACION_EXC.get(tramiteKey);
                boolean auditarExcepcion = (ultimaExcepcion == null || minutoActual.compareTo(ultimaExcepcion) != 0);
                if (auditarExcepcion) {
                    LIMITES_TRAMITACION_EXC.put(tramiteKey, minutoActual);
                }
                // Generamos excepcion
                throw new LimiteTramitacionException(idTramite, version, limitNumero, auditarExcepcion);
            }
    ...
    /**
     * Obtiene hora con minuto actual (a 0 segs).
     *
     * @return hora con minuto actual (a 0 segs)
     */
    private static Date getMinutoActual() {
