...
public class UtilsFormulario {
    ...
    public static XmlFormulario xmlToValores(final byte[] pXml) {
        ...
        try {
            if (pXml != null) {
                ...
                final FORMULARIODocument doc = FORMULARIODocument.Factory
                        .parse(xmlStr);
                ...
                for (final CAMPO campo : doc.getFORMULARIO().getCAMPOArray()) {
                    if (campo.getTipo() == CAMPO.Tipo.SIMPLE) {
                        final ValorCampoSimple vcfs = ValorCampoSimple
                                .createNewValorCampoSimple();
                    ...
                    if (campo.getTipo() == CAMPO.Tipo.COMPUESTO) {
                        final ValorCampoIndexado vcfi = ValorCampoIndexado
                                .createNewValorCampoIndexado();
                        vcfi.setId(campo.getId());
                        if (campo.getVALORArray().length > 0) {
                            vcfi.setValor(ValorIndexado.createNewValorIndexado(
                                    campo.getVALORArray()[0].getCodigo(),
                                    campo.getVALORArray()[0].getStringValue()));
                        }
                        datos.add(vcfi);
                    }
                    ...
                    if (campo.getTipo() == CAMPO.Tipo.MULTIVALUADO) {
                        ...
                        for (final VALOR valor : campo.getVALORArray()) {
                            lstValor.addValorIndexado(valor.getCodigo(),
            ...
            throw new XmlFormularioException("Encoding UTF-8 no soportado",
                    uee);
     ...
     * @return XML
     */
    private static byte[] generarXml(final String pAccionPersonalizada,
            final List<ValorCampo> pCampos) {
        final XmlOptions xmlOptions = new XmlOptions()
                .setCharacterEncoding(Constantes.UTF8);
        ...
        for (final ValorCampo valorCampo : pCampos) {
            ...
            case INDEXADO:
                ...
                if (vci.getValor() != null) {
                    ...
                    valorIndexadoXml
                            .setStringValue(vci.getValor().getDescripcion());
                }
                break;
            case LISTA_INDEXADOS:
                ...
                if (vcl.getValor() != null) {
                    for (final ValorIndexado vi : vcl.getValor()) {
                        final VALOR valorListaIndexadoXml = campoXml
                                .addNewVALOR();
                        valorListaIndexadoXml.setCodigo(vi.getValor());
                        valorListaIndexadoXml
                                .setStringValue(vi.getDescripcion());
                    }
                }
                break;
            default:
                throw new TipoNoControladoException(
                        "Tipo de campo no permitido: "
                                + valorCampo.getTipo().name());
            }
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream(
                pCampos.size() * ConstantesNumero.N60)) {
