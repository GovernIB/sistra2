...
public final class ControladorPasoPagarHelper {
	 ...
	 */
	public String generarMensajeEstadoIncorrecto(final Literales literales, final String idioma,
			final TypeEstadoPagoIncorrecto estadoPago, final String codErrorPasarela, final String msgErrorPasarela) {
		String res = "";
		if (estadoPago != null) {
	 ...
	 */
	public DatosSesionPago fromXML(final byte[] content) {
		try {
			...
			res.setPresentacion(TypePresentacion.fromString(doc.getPAGO().getDATOSPAGO().getPRESENTACION()));
	 ...
	 * @return XML pago
	 */
	public byte[] toXML(final DatosSesionPago datosSesionPago) {
		try {
			...
			datosPago.setPRESENTACION(datosSesionPago.getPresentacion().toString());
			...
			contribuyente.setNOMBRE(datosSesionPago.getSujetoPasivo().getNombre());
	 ...
	 */
	public DatosSesionPago crearSesionPagoPasarela(final TypePresentacion presentacion,
			final String idSesionTramitacion, final DatosInternosPasoPagar pDipa, final String idPago,
			final String pIdioma, final DefinicionTramiteSTG pDefinicionTramite) {
	 ...
	 */
	private Date stringToDate(final String fecha) throws ParseException {
