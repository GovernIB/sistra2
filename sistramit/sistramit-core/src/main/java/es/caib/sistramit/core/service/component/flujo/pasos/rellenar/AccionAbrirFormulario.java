...
public final class AccionAbrirFormulario implements AccionPaso {
    ...
    public RespuestaEjecutarAccionPaso ejecutarAccionPaso(DatosPaso pDatosPaso,
        ...
        final String idFor = (String) UtilsFlujo.recuperaParametroAccionPaso(
                pParametros, PARAM_ID_FORMULARIO, true);
        ...
        final Formulario formularioDetalle = ((DetallePasoRellenar) dipa
                .getDetallePaso()).getFormulario(idFor);
        if (formularioDetalle == null) {
            throw new AccionPasoNoPermitidaException("No existe formulario "
                    + idFor + " en paso " + dipa.getIdPaso());
        }
        if (formularioDetalle
                .getObligatorio() == TypeObligatoriedad.DEPENDIENTE) {
            throw new AccionPasoNoPermitidaException(
                    "Un formulario dependiente no puede ser abierto");
        }
        ...
        final RFormularioTramite formularioDef = UtilsSTG
                .devuelveDefinicionFormulario(pasoDef, idFor);

        // Comprobamos si el formulario tiene parametros de apertura
        final Map<String, String> parametros = calcularParametrosApertura(dipa,
                pDpp, pDefinicionTramite, formularioDef, pVariablesFlujo);

        // Generamos sesion en el controlador de formularios (interno o externo)
        final AbrirFormulario af = generarSesionGestorFormulario(dipa,
                pDefinicionTramite, formularioDef, pVariablesFlujo, parametros);
     ...
     */
    private Map<String, String> calcularParametrosApertura(
        ...
        if (UtilsSTG
                .existeScript(pFormularioDef.getScriptParametrosApertura())) {
            ...
            final Map<String, String> codigosError = UtilsSTG
                    .convertLiteralesToMap(pFormularioDef
                            .getScriptParametrosApertura().getLiterales());
            final RespuestaScript rs = scriptFlj.executeScriptFlujo(
                    TypeScriptFlujo.SCRIPT_PARAMETROS_FORMULARIO,
                    pFormularioDef.getIdentificador(),
                    pFormularioDef.getScriptParametrosApertura().getScript(),
                    pVariablesFlujo, null, documentosPaso, codigosError,
                    pDefinicionTramite);
            if (rs.isError()) {
                throw new ErrorScriptException(
                        TypeScriptFlujo.SCRIPT_PARAMETROS_FORMULARIO.name(),
                        pVariablesFlujo.getIdSesionTramitacion(),
                        pFormularioDef.getIdentificador(),
                        rs.getMensajeError());
            }
            // Establecemos parametros
            final ResParametrosFormulario rsp = (ResParametrosFormulario) rs
                    .getResultado();
     ...
     */
    private AbrirFormulario generarSesionGestorFormulario(
            final DatosInternosPasoRellenar pDipa,
            final DefinicionTramiteSTG pDefinicionTramite,
            final RFormularioTramite pDefinicionFormulario,
            final VariablesFlujo pVariablesFlujo,
            final Map<String, String> pParametrosApertura) {

        final DatosInicioSesionFormulario difi = new DatosInicioSesionFormulario();
        difi.setInfoTramiteCP(pVariablesFlujo.getDatosTramiteCP());
        difi.setIdioma(pVariablesFlujo.getIdioma());
        difi.setIdTramite(
                pDefinicionTramite.getDefinicionVersion().getIdentificador());
        difi.setVersionTramite(
                pDefinicionTramite.getDefinicionVersion().getVersion());
        difi.setReleaseTramite(
                pDefinicionTramite.getDefinicionVersion().getRelease());
        ...
        difi.setNivelAutenticacion(pVariablesFlujo.getNivelAutenticacion());
        difi.setUsuario(pVariablesFlujo.getUsuario());
        difi.setXmlDatosActuales(pDipa
                .getDatosFormulario(pDefinicionFormulario.getIdentificador()));
        ...
        if (pDefinicionFormulario.isInterno()) {
            ...
            urlFormulario = controladorGFInterno.iniciarSesion(difi);
        } else {
            // TODO PENDIENTE
