...
public final class FlujoPasoDaoImpl implements FlujoPasoDao {
    ...
    @Override
    public DatosPersistenciaPaso obtenerPasoPersistencia(
            final String pIdSesionTramitacion, final String pIdPaso) {
        ...
        for (final HDocumento hDoc : hPaso.getDocumentos()) {
            ...
            final DocumentoPasoPersistencia documento = HDocumento.toModel(hDoc,
                    contador++);
    ...
    @Override
    public void cambiarEstadoPaso(final String pIdSesionTramitacion,
            final String pIdPaso, final TypeEstadoPaso pEstado) {
        final HPaso hPaso = getHPaso(pIdSesionTramitacion, pIdPaso);
    ...
    @Override
    public void establecerDatosDocumento(final String pIdSesionTramitacion,
            final String pIdPaso, final DocumentoPasoPersistencia pDocPaso) {
        // Recuperamos paso
        final HPaso hPaso = getHPaso(pIdSesionTramitacion, pIdPaso);

        // Comprobamos si existe documento
        HDocumento hdoc = findHDocumento(hPaso.getCodigo(), pDocPaso.getId(),
                pDocPaso.getInstancia());
        if (hdoc != null) {
    ...
    @Override
    public void eliminarDocumento(final String pIdSesionTramitacion,
            final String pIdPaso, final String pIdDocumento,
            final int pInstancia) {
        final HPaso hPaso = getHPaso(pIdSesionTramitacion, pIdPaso);
        final HDocumento hdoc = findHDocumento(hPaso.getCodigo(), pIdDocumento,
                pInstancia);
        // Borramos documento
    ...
    @Override
    public ReferenciaFichero insertarFicheroPersistencia(final String pNombre,
            final byte[] pContenido, final String pIdSesionTramitacion) {
        ...
        if (StringUtils.isBlank(pNombre)) {
            throw new RepositoryException(
                    "No se ha indicado nombre del fichero");
        }
    ...
    @Override
    public DatosFicheroPersistencia recuperarFicheroPersistencia(
            final ReferenciaFichero pRefFic) {
    ...
    @Override
    public void actualizarFicheroPersistencia(final ReferenciaFichero pRefFic,
            final String pNombre, final byte[] pContenido) {
    ...
    @Override
    public DocumentoPasoPersistencia obtenerDocumentoPersistencia(
            final String pIdSesionTramitacion, final String pIdPaso,
            final String pIdAnexo, final int pInstancia) {
        final HPaso hPaso = getHPaso(pIdSesionTramitacion, pIdPaso);
        final HDocumento hDocumento = getHDocumento(hPaso.getCodigo(), pIdAnexo,
                pInstancia);
        final DocumentoPasoPersistencia dpp = HDocumento.toModel(hDocumento,
                pInstancia);
        return dpp;
    }
     ...
     * @return documento
     */
    private HDocumento findHDocumento(Long codPaso, String idDocumento,
            int instancia) {
     ...
     * @return documento
     */
    private HDocumento getHDocumento(Long codPaso, String idDocumento,
            int instancia) {
        final HDocumento hDoc = findHDocumento(codPaso, idDocumento, instancia);
        if (hDoc == null) {
            throw new RepositoryException(
                    "No se encuentra documento " + idDocumento + " - "
                            + instancia + " en el paso " + codPaso);
        }
        return hDoc;
    }
     ...
     * @return HFichero
     */
    private HFichero findHFichero(ReferenciaFichero referenciaFichero) {
     ...
     * @return HFichero
     */
    private HFichero getHFichero(ReferenciaFichero referenciaFichero) {
        final HFichero hFichero = findHFichero(referenciaFichero);
        if (hFichero == null) {
            throw new RepositoryException(
                    "No se encuentra fichero " + referenciaFichero.getId()
                            + " - " + referenciaFichero.getClave());
        }
        return hFichero;
    }
    ...
    private String generarClave() {
        ...
        for (int i = 0; i < ConstantesNumero.N10; i++) {
            clave.append(Character.forDigit(
                    ca + r.nextInt(ConstantesNumero.N26), Character.MAX_RADIX));
        }
