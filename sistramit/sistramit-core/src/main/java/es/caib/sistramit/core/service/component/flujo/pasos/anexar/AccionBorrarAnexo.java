...
public final class AccionBorrarAnexo implements AccionPaso {
	...
	private void actualizarPersistencia(final DatosInternosPasoAnexar pDipa, final DatosPersistenciaPaso pDpp,
		...
		listaFicherosBorrar.addAll(docPersistencia.obtenerReferenciasFicherosAnexo(true, true));
		...
		if (pDpp.getNumeroInstanciasDocumento(pIdAnexo) > ConstantesNumero.N1) {
			...
			dao.eliminarDocumento(pVariablesFlujo.getIdSesionTramitacion(), pDipa.getIdPaso(), pIdAnexo, pInstancia);
			...
			docPersistencia.removeFirmasFicheros();

			dao.establecerDatosDocumento(pVariablesFlujo.getIdSesionTramitacion(), pDipa.getIdPaso(), docPersistencia);
		}
	...
	private void actualizarDetalleAnexo(final DatosInternosPasoAnexar pDipa, final String pIdAnexo,
		...
		final Anexo anexoDetalle = ((DetallePasoAnexar) pDipa.getDetallePaso()).getAnexo(pIdAnexo);
