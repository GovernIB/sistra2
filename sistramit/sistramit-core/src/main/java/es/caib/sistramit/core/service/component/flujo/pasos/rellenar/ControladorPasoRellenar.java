...
public final class ControladorPasoRellenar
    ...
    protected List<DatosDocumento> obtenerDocumentosCompletadosPaso(
        ...
        final DatosInternosPasoRellenar dipa = ((DatosInternosPasoRellenar) pDatosPaso
                .internalData());
        return UtilsPasoRellenar.obtenerDocumentosCompletados(dipa, pDpp,
                pDefinicionTramite, null);
    }
    ...
    private void regenerarDatos(DatosInternosPasoRellenar pDipa,
        ...
        final DetallePasoRellenar dpaNew = calcularDetalle(pDipa.getIdPaso(),
                pDefinicionTramite, pVariablesFlujo, pDpp);
        ...
        regenerarDatosDocumentosNuevos(pDipa, pDpp, pDefinicionTramite,
                pVariablesFlujo);

    }

    /**
     * Método para calcular el detalle del paso en base a la definición del
     * trámite.
     *
     * @param idPaso
     *            Parámetro id paso
     ...
     */
    private DetallePasoRellenar calcularDetalle(final String idPaso,
        ...
        final RPasoTramitacionRellenar defPaso = (RPasoTramitacionRellenar) UtilsSTG
                .devuelveDefinicionPaso(idPaso, pDefinicionTramite);

        // Recorremos la lista fija de documentos
        final List<Formulario> formularios = calcularDetalleListaFijaFormularios(
                pDefinicionTramite, defPaso, pVariablesFlujo, pDpp);
     ...
     */
    private List<Formulario> calcularDetalleListaFijaFormularios(
            final DefinicionTramiteSTG pDefinicionTramite,
        ...
        for (final RFormularioTramite formularioDef : pPasoDef
            ...
            final Formulario formulario = calcularDetalleFormularioFijo(
                    pVariablesFlujo, formulariosCompletados, pDefinicionTramite,
                    formularioDef);
            ...
            final DocumentoPasoPersistencia docPer = pDpp
                    .getDocumentoPasoPersistencia(formulario.getId(),
                            ConstantesNumero.N1);
            if (formulario.getObligatorio() != TypeObligatoriedad.DEPENDIENTE
                    && docPer != null && docPer
                            .getEstado() == TypeEstadoDocumento.RELLENADO_CORRECTAMENTE) {
                // Recuperamos el xml del formulario de persistencia
                final DatosFicheroPersistencia ficPer = getDao()
                        .recuperarFicheroPersistencia(docPer.getFichero());
                // Creamos datos formulario y lo añadimos a lista de formularios
                // completados
                final DatosDocumentoFormulario ddf = UtilsPasoRellenar
                        .crearDatosDocumentoFormulario(pDpp, formulario,
                                ValoresFormulario.createValoresFormulario(
                                        ficPer.getContenido()));
     ...
     */
    private Formulario calcularDetalleFormularioFijo(
            final VariablesFlujo pVariablesFlujo,
        ...
        final TypeObligatoriedad rs = calcularObligatoriedadFormularioFijo(
                pDefinicionTramite, pFormularioDef, pFormulariosCompletados,
                pVariablesFlujo);
        formulario.setObligatorio(rs);

        // Evaluamos si se debe firmar digitalmente
        if (pFormularioDef.isFirmar() && formulario
            ...
            if (pFormularioDef.getScriptFirmantes() != null && !StringUtils
                    .isEmpty(pFormularioDef.getScriptFirmantes().getScript())) {
                final ResFirmantes resf = ejecutarScriptFirmantes(
                        pVariablesFlujo, pDefinicionTramite, pFormularioDef,
                ...
                final Persona f = new Persona(
                        pVariablesFlujo.getUsuario().getNif(),
     ...
     */
    private TypeObligatoriedad calcularObligatoriedadFormularioFijo(
            final DefinicionTramiteSTG pDefinicionTramite,
            final RFormularioTramite pFormularioDef,
            final List<DatosDocumento> pFormulariosCompletados,
            final VariablesFlujo pVariablesFlujo) {
        ...
        TypeObligatoriedad rs = TypeObligatoriedad
                .fromString(pFormularioDef.getObligatoriedad());
        if (rs == null) {
            throw new TipoNoControladoException(
                    "Valor " + pFormularioDef.getObligatoriedad()
        ...
        if (rs == TypeObligatoriedad.OPCIONAL) {
            rs = UtilsFlujo.ejecutarScriptDependenciaDocumento(
                    ...
                    pFormulariosCompletados, getScriptFlujo(),
                    pDefinicionTramite);
        }
        return rs;
    }
     ...
     */
    private ResFirmantes ejecutarScriptFirmantes(
            final VariablesFlujo pVariablesFlujo,
            final DefinicionTramiteSTG pDefinicionTramite,
            final RFormularioTramite formularioDef,
            final List<DatosDocumento> pFormulariosCompletados) {

        // Ejecutamos script
        final Map<String, String> codigosError = UtilsSTG.convertLiteralesToMap(
                formularioDef.getScriptFirmantes().getLiterales());
        final RespuestaScript rs = getScriptFlujo().executeScriptFlujo(
                TypeScriptFlujo.SCRIPT_FIRMANTES,
                formularioDef.getIdentificador(),
                formularioDef.getScriptFirmantes().getScript(), pVariablesFlujo,
                null, pFormulariosCompletados, codigosError,
                pDefinicionTramite);
        if (rs.isError()) {
            throw new ErrorScriptException(
                    TypeScriptFlujo.SCRIPT_FIRMANTES.name(),
                    pVariablesFlujo.getIdSesionTramitacion(),
                    formularioDef.getIdentificador(), rs.getMensajeError());
        }
     ...
     *            Variables flujo
     */
    private void regenerarDatosDocumentosExistentes(
        ...
        for (final DocumentoPasoPersistencia docDpp : pDpp.getDocumentos()) {
            ...
            if (((DetallePasoRellenar) pDipa.getDetallePaso())
                    .getFormulario(docDpp.getId()) == null) {
                throw new ConfiguracionModificadaException("El formulario "
                        + docDpp.getId() + " no existe en configuración");
            }

            // Obtenemos detalle formulario recalculado
            final Formulario detalleForm = ((DetallePasoRellenar) pDipa
                    .getDetallePaso()).getFormulario(docDpp.getId());

            // Regeneramos doc existente
            regenerarDocumentoExistente(pDipa.getIdPaso(), detalleForm, docDpp,
                    pVariablesFlujo);

            // Cacheamos datos formulario
            final DatosFicheroPersistencia dfp = getDao()
                    .recuperarFicheroPersistencia(docDpp.getFichero());
     ...
     *            Variables flujo
     */
    private void regenerarDocumentoExistente(final String pIdPaso,
            final Formulario pDetalleFormulario,
            final DocumentoPasoPersistencia pDocDpp,
            final VariablesFlujo pVariablesFlujo) {

        // Actualiza persistencia en funcion del detalle de formulario
        // recalculado
        actualizarPersistenciaExistente(pIdPaso, pDocDpp, pDetalleFormulario,
                pVariablesFlujo);
     ...
     */
    private void actualizarDetalleExistente(final Formulario detalleForm,
            final DocumentoPasoPersistencia pDocDpp) {
     ...
     *            Variables flujo
     */
    private void actualizarPersistenciaExistente(final String idPaso,
            final DocumentoPasoPersistencia pDocDpp,
            final Formulario detalleForm,
            final VariablesFlujo pVariablesFlujo) {
     ...
     *            ficheros a borrar
     */
    private void updateDB(final String idPaso,
            final DocumentoPasoPersistencia pDocDpp,
            final VariablesFlujo pVariablesFlujo,
            final List<ReferenciaFichero> ficsPersistenciaBorrar) {
        getDao().establecerDatosDocumento(
                pVariablesFlujo.getIdSesionTramitacion(), idPaso, pDocDpp);
    ...
    /**
     * Creamos en persistencia los documentos que existen en el detalle pero que
     * aun no existen en persistencia (ocurrirá al inicializar paso).
     *
     * @param pDipa
     *            Datos internos paso
     * @param pDpp
     ...
     *            Variables flujo
     */
    private void regenerarDatosDocumentosNuevos(
            final DatosInternosPasoRellenar pDipa,
            final DatosPersistenciaPaso pDpp,
            final DefinicionTramiteSTG pDefinicionTramite,
            final VariablesFlujo pVariablesFlujo) {
        ...
        for (final Formulario formulario : ((DetallePasoRellenar) pDipa
                .getDetallePaso()).getFormularios()) {
            if (pDpp.getNumeroInstanciasDocumento(formulario.getId()) == 0) {
                // Obtenemos datos iniciales formulario
                final byte[] xml = obtenerDatosIniciales(pVariablesFlujo,
                        pDefinicionTramite, pasoDef, formulario);

                // Cacheamos datos formulario
                pDipa.addDatosFormulario(formulario.getId(), xml);

                // Creamos documento en BBDD
                final ReferenciaFichero refXml = getDao()
                        .insertarFicheroPersistencia(
                                formulario.getId() + ".xml", xml,
                                pVariablesFlujo.getIdSesionTramitacion());
    ...
    /**
     * Método para Obtener datos iniciales de la clase
     * ControladorPasoRellenarImpl.
     *
     * @param pVariablesFlujo
    ...
    private byte[] obtenerDatosIniciales(final VariablesFlujo pVariablesFlujo,
            final DefinicionTramiteSTG pDefinicionTramite,
            final RPasoTramitacionRellenar pasoDef,
            final Formulario formulario) {
        // Inicializamos datos del formulario (script datos iniciales)
        final ValoresFormulario vf = ValoresFormulario
                .createValoresFormularioVacio();
        final RFormularioTramite formDef = UtilsSTG
                .devuelveDefinicionFormulario(pasoDef, formulario.getId());
        if (UtilsSTG.existeScript(formDef.getScriptDatosIniciales())) {
            ...
            final Map<String, String> codigosError = UtilsSTG
                    .convertLiteralesToMap(
                            formDef.getScriptDatosIniciales().getLiterales());
            final RespuestaScript rs = getScriptFlujo().executeScriptFlujo(
                    TypeScriptFlujo.SCRIPT_DATOS_INICIALES_FORMULARIO,
                    formDef.getIdentificador(),
                    formDef.getScriptDatosIniciales().getScript(),
            ...
            if (rs.isError()) {
                throw new ErrorScriptException(
                        TypeScriptFlujo.SCRIPT_DATOS_INICIALES_FORMULARIO
                                .name(),
                        pVariablesFlujo.getIdSesionTramitacion(),
                        formDef.getIdentificador(), rs.getMensajeError());
            }

            // Recuperamos datos iniciales
            final ResDatosInicialesFormulario rsd = (ResDatosInicialesFormulario) rs
                    .getResultado();
