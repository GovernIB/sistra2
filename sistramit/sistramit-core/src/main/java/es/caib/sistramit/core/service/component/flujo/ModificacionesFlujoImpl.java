...
public final class ModificacionesFlujoImpl implements ModificacionesFlujo {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(ModificacionesFlujoImpl.class);
    ...
    @Override
    public VariablesFlujo generarVariablesFlujo(
        ...
        vf.setIdTramite(pDatosSesion.getDefinicionTramite()
                .getDefinicionVersion().getIdentificador());
        vf.setVersionTramite(pDatosSesion.getDefinicionTramite()
                .getDefinicionVersion().getVersion());
        vf.setIdSesionTramitacion(
        ...
        vf.setDatosTramiteCP(
                pDatosSesion.getDatosTramite().getDefinicionTramiteCP());

        vf.setMetodoAutenticacion(
                pDatosSesion.getDatosTramite().getMetodoAutenticacionInicio());
        vf.setNivelAutenticacion(
                pDatosSesion.getDatosTramite().getNivelAutenticacion());

        DatosUsuario du = null;
        if (pDatosSesion.getDatosTramite()
                .getNivelAutenticacion() == TypeAutenticacion.AUTENTICADO) {
            du = new DatosUsuario();
            du.setNif(pDatosSesion.getDatosTramite().getIniciador().getNif());
            du.setNombre(
                    pDatosSesion.getDatosTramite().getIniciador().getNombre());
            du.setApellido1(pDatosSesion.getDatosTramite().getIniciador()
                    .getApellido1());
        ...
        vf.setUsuario(du);
        vf.setUsuarioAutenticado(
                pDatosSesion.getDatosTramite().getUsuarioAutenticado());
        vf.setParametrosInicio(
                pDatosSesion.getDatosTramite().getParametrosInicio());
        ...
        if (idPaso != null) {
            ...
            for (final DatosPaso dp : pDatosSesion.getDatosTramite()
                    .getDatosPasos()) {
                ...
                if (idPaso.equals(dp.getIdPaso())) {
                    ...
                    if (dp.getEstado() == TypeEstadoPaso.COMPLETADO) {
                        final ControladorPaso cp = getControladorPaso(
                                pDatosSesion, dp.getIdPaso());
                        final List<DatosDocumento> docsPaso = cp
                                .obtenerDocumentosPaso(dp,
    ...
    public void actualizarEstadoTramite(
        ...
        if (dpUltimo != null
                && dpUltimo.getEstado() == TypeEstadoPaso.COMPLETADO
                && dpUltimo.isPasoFinal()) {
        ...
        if (datosSesion.getDatosTramite().getEstadoTramite() != estadoNuevo
                && datosSesion.getDatosTramite()
                        .getEstadoTramite() != TypeEstadoTramite.FINALIZADO) {
            datosSesion.getDatosTramite().setEstadoTramite(estadoNuevo);
            dao.cambiaEstadoTramite(
    ...
    @Override
    public void personalizarTramite(final DatosSesionTramitacion pDatosSesion,
            final ResPersonalizacionTramite rp) {
        // Personalizacion titulo tramite
        if (StringUtils.isNotBlank(rp.getTituloTramite())) {
            pDatosSesion.getDatosTramite()
                    .setTituloTramite(rp.getTituloTramite());
        }
        ...
        final Date fcFin = UtilsFlujo.deformateaFecha(rp.getPlazoFin(),
                ResPersonalizacionTramiteInt.FORMATO_FECHA);
    ...
    @Override
    public RespuestaAccionPaso invocarAccionPaso(
        ...
        final DatosPaso dp = pDatosSesion.getDatosTramite()
                .getDatosPaso(pIdPaso);
        // Generamos variables flujo accesibles desde el paso
        final VariablesFlujo variablesFlujo = this
                .generarVariablesFlujo(pDatosSesion, pIdPaso);
        final ControladorPaso cp = this.getControladorPaso(pDatosSesion,
                pIdPaso);
        final RespuestaAccionPaso res = cp.accionPaso(dp, pAccionPaso,
                pParametros, pDatosSesion.getDefinicionTramite(),
                variablesFlujo);
        return res;
    }

    @Override
    public void crearTramitePersistencia(
        ...
        dpdt.setParametrosInicio(
                pDatosSesion.getDatosTramite().getParametrosInicio());
        dpdt.setIdSesionTramitacion(
                pDatosSesion.getDatosTramite().getIdSesionTramitacion());
        dpdt.setEstado(pDatosSesion.getDatosTramite().getEstadoTramite());
        dpdt.setDescripcionTramite(
                pDatosSesion.getDatosTramite().getTituloTramite());
        dpdt.setIdioma(pDatosSesion.getDatosTramite().getIdioma());
        dpdt.setVersionTramite(
                pDatosSesion.getDatosTramite().getVersionTramite());
        dpdt.setIdTramite(pDatosSesion.getDatosTramite().getIdTramite());
        dpdt.setIdArea(pDatosSesion.getDefinicionTramite()
                .getDefinicionVersion().getIdArea());
        dpdt.setIdTramiteCP(pDatosSesion.getDatosTramite()
                .getDefinicionTramiteCP().getIdentificador());
        dpdt.setIdProcedimientoCP(pDatosSesion.getDatosTramite()
                .getDefinicionTramiteCP().getIdentificador());
        dpdt.setIdProcedimientoSIA(
                pDatosSesion.getDatosTramite().getDefinicionTramiteCP()
                        .getProcedimiento().getIdProcedimientoSIA());
        dpdt.setAutenticacion(
                pDatosSesion.getDatosTramite().getNivelAutenticacion());
        dpdt.setMetodoAutenticacionInicio(
                pDatosSesion.getDatosTramite().getMetodoAutenticacionInicio());
        if (pDatosSesion.getDatosTramite().getIniciador() != null) {
            dpdt.setNifIniciador(
                    pDatosSesion.getDatosTramite().getIniciador().getNif());
            dpdt.setNombreIniciador(
                    pDatosSesion.getDatosTramite().getIniciador().getNombre());
            dpdt.setApellido1Iniciador(pDatosSesion.getDatosTramite()
                    .getIniciador().getApellido1());
            dpdt.setApellido2Iniciador(pDatosSesion.getDatosTramite()
                    .getIniciador().getApellido2());
        }
        dpdt.setTimestamp(new Date().getTime());
        dpdt.setFechaInicio(new Date());
        dpdt.setPersistente(pDatosSesion.getDefinicionTramite()
                .getDefinicionVersion().getPropiedades().isPersistente());
    ...
    @Override
    public void addPasoTramitacion(final DatosSesionTramitacion pDatosSesion,
            final String idPaso, final TypePaso tipo) {
        final DatosPaso dp = DatosPaso.createNewDatosPaso(
                pDatosSesion.getDatosTramite().getIdSesionTramitacion(), idPaso,
                tipo);
        pDatosSesion.getDatosTramite().getDatosPasos().add(dp);
        dao.crearPaso(pDatosSesion.getDatosTramite().getIdSesionTramitacion(),
                dp.getIdPaso(), dp.getTipo(),
                pDatosSesion.getDatosTramite().getDatosPasos().size());
    }

    @Override
    public void removeUltimoPasoTramitacion(
            final DatosSesionTramitacion pDatosSesion) {
        final DatosPaso dp = pDatosSesion.getDatosTramite()
                .getDatosPasoUltimo();
    ...
    @Override
    public void invalidarPasoTramitacion(
            final DatosSesionTramitacion pDatosSesion, final String idPaso) {
        final DatosPaso dp = pDatosSesion.getDatosTramite()
                .getDatosPaso(idPaso);
        final ControladorPaso cp = this.getControladorPaso(pDatosSesion,
                idPaso);
        final VariablesFlujo variablesFlujo = this
                .generarVariablesFlujo(pDatosSesion, idPaso);
        cp.invalidarPaso(dp, pDatosSesion.getDefinicionTramite(),
                variablesFlujo);
    }

    @Override
    public void establecerAccesibilidadPaso(
            final DatosSesionTramitacion pDatosSesion, final String idPaso,
    ...
    @Override
    public void establecerSoloLecturaPaso(
            final DatosSesionTramitacion pDatosSesion, final String idPaso,
        ...
        final ControladorPaso cp = this.getControladorPaso(pDatosSesion,
                idPaso);
        cp.establecerSoloLectura(dp, pDatosSesion.getDefinicionTramite(),
                soloLectura);
    }

    @Override
    public void cargarPasosPersistencia(
            final DatosSesionTramitacion pDatosSesion) {
        // Obtiene lista de pasos de persistencia
        final List<EstadoPersistenciaPasoTramite> pasosPersist = dao
                .obtenerListaPasos(pDatosSesion.getDatosTramite()
                        .getIdSesionTramitacion());
        String idPasoActual = null;
        for (final EstadoPersistenciaPasoTramite ept : pasosPersist) {
            // Creamos datos del paso
            pDatosSesion.getDatosTramite().getDatosPasos()
                    .add(DatosPaso.createNewDatosPaso(
                            pDatosSesion.getDatosTramite()
                                    .getIdSesionTramitacion(),
                            ept.getId(), ept.getTipo()));
            // Comprobamos si esta pendiente o completado
            if (ept.getEstado() != TypeEstadoPaso.NO_INICIALIZADO
                    && ept.getEstado() != TypeEstadoPaso.REVISAR) {
        ...
        if (idPasoActual == null) {
            if (pasosPersist.isEmpty()) {
                throw new ErrorConfiguracionException(
                        "No se ha recuperado ningun paso de persistencia");
            }
        ...
        for (final DatosPaso dp : pDatosSesion.getDatosTramite()
            ...
            // Cargamos paso
            final VariablesFlujo variablesFlujo = this
                    .generarVariablesFlujo(pDatosSesion, dp.getIdPaso());
            this.getControladorPaso(pDatosSesion, dp.getIdPaso()).cargarPaso(dp,
                    pDatosSesion.getDefinicionTramite(), variablesFlujo);
        }

    }

    @Override
    public void registrarAccesoTramite(
            final DatosSesionTramitacion pDatosSesion) {
        // Calculamos fecha de caducidad
        Date fechaCaducidad = null;

        // Obtenemos los días de persistencia de la definición del trámite
        final RVersionTramitePropiedades propiedades = pDatosSesion
                .getDefinicionTramite().getDefinicionVersion().getPropiedades();
        ...
        if (diasPersistencia > 0) {
            ...
            if (pDatosSesion.getDatosTramite().getPlazoFin() != null) {
                ...
                if (cal.getTime()
                        .after(pDatosSesion.getDatosTramite().getPlazoFin())) {
                    fechaCaducidad = pDatosSesion.getDatosTramite()
                            .getPlazoFin();
                } else {
        ...
        // Registramos acceso
        final Date timestampFlujo = dao.registraAccesoTramite(
                pDatosSesion.getDatosTramite().getIdSesionTramitacion(),
                fechaCaducidad);
    ...
    @Override
    public boolean establecerPasoActual(
            final DatosSesionTramitacion pDatosSesion, final String pIdPaso) {
        ...
        if (pDatosSesion.getDatosTramite().getIdPasoActual() == null
                || pDatosSesion.getDatosTramite()
                        .getAccesibilidadPaso(pIdPaso)) {
            // Verificamos si hay que inicializar o revisar el paso
            final DatosPaso dpDest = pDatosSesion.getDatosTramite()
                    .getDatosPaso(pIdPaso);
            if (dpDest == null) {
                throw new AccionPasoNoPermitidaException(
                        "No existen datos para paso " + pIdPaso);
            }
            if (dpDest.getEstado() == TypeEstadoPaso.NO_INICIALIZADO
                    || dpDest.getEstado() == TypeEstadoPaso.REVISAR) {
                // Preparamos plugins paso
                final VariablesFlujo variablesPaso = this
                        .generarVariablesFlujo(pDatosSesion, pIdPaso);
                // Inicializamos o revisamos paso
                if (dpDest.getEstado() == TypeEstadoPaso.NO_INICIALIZADO) {
                    this.getControladorPaso(pDatosSesion, pIdPaso)
                            .inicializarPaso(dpDest,
                                    pDatosSesion.getDefinicionTramite(),
                                    variablesPaso);
                } else {
                    this.getControladorPaso(pDatosSesion, pIdPaso).revisarPaso(
                            dpDest, pDatosSesion.getDefinicionTramite(),
                            variablesPaso);
                }
            }
        ...
        return (pDatosSesion.getDatosTramite().getIdPasoActual()
                .equals(pIdPaso));
    }

    @Override
    public DetallePaso obtenerDetallePaso(
            final DatosSesionTramitacion pDatosSesion, final String pIdPaso) {
        final DatosPaso datosPaso = pDatosSesion.getDatosTramite()
                .getDatosPaso(pIdPaso);
        if (datosPaso == null) {
            throw new AccionPasoNoPermitidaException(
                    "No existen datos del paso " + pIdPaso);
        }
        final ControladorPaso cpa = this.getControladorPaso(pDatosSesion,
                pIdPaso);
        final DetallePaso dtpa = cpa.detallePaso(datosPaso,
                pDatosSesion.getDefinicionTramite());
        return dtpa;
    }
    ...
    public void calcularSiguientePaso(
        ...
        final DatosPaso datosPasoActual = pDatosSesion.getDatosTramite()
                .getDatosPasoActual();
        ...
        if (datosPasoActual.getEstado() == TypeEstadoPaso.COMPLETADO) {
            ...
            final RPasoTramitacion defPasoActual = UtilsSTG
                    .devuelveDefinicionPaso(idPasoActual,
                            pDatosSesion.getDefinicionTramite());

            // Ejecutamos script de navegacion
            idPasoSiguiente = ejecutarScriptNavegacion(pDatosSesion,
                    datosPasoActual.getIdPaso(), defPasoActual);

            // Si no se ha establecido idpaso el siguiente paso sera el
            // siguiente en la definición
            if (idPasoSiguiente == null) {
                // Si el paso actual es registrar pasamos a paso virtual de
                // guardar
                final TypePaso tipoPasoActual = TypePaso
                        .fromString(defPasoActual.getTipo());
                if (tipoPasoActual == TypePaso.REGISTRAR) {
                    ...
                    final int indicePasoActual = UtilsSTG.devuelveIndicePaso(
                            idPasoActual, pDatosSesion.getDefinicionTramite());
                    if (indicePasoActual == ConstantesNumero.N_1) {
                        throw new ErrorConfiguracionException(
                                "No se encuentra paso " + idPasoActual);
                    }

                    final int indiceSiguiente = indicePasoActual
                            + ConstantesNumero.N1;

                    if (indiceSiguiente >= pDatosSesion.getDefinicionTramite()
                            .getDefinicionVersion().getPasos().size()) {
                        throw new ErrorConfiguracionException("El paso "
                                + idPasoActual
                                + " es el último, no es final y no tiene configurado un paso siguiente en el script de navegacion");
                    }
                    idPasoSiguiente = pDatosSesion.getDefinicionTramite()
    ...
    @Override
    public RespuestaScript ejecutarScriptPersonalizacion(
            final DatosSesionTramitacion pDatosSesion) {
        RespuestaScript rs = null;
        final RVersionTramitePropiedades propiedadesTramite = pDatosSesion
                .getDefinicionTramite().getDefinicionVersion().getPropiedades();
        if (UtilsSTG
                .existeScript(propiedadesTramite.getScriptPersonalizacion())) {
            ...
            rs = this.scriptFlujo.executeScriptFlujo(
                    TypeScriptFlujo.SCRIPT_PERSONALIZACION_TRAMITE, null,
                    script, variablesFlujo, null, null, codigosError,
                    pDatosSesion.getDefinicionTramite());
        }
        return rs;
    }
    ...
    @Override
    public void calcularParametrosInicialesTramite(
            final DatosSesionTramitacion pDatosSesion) {
        RespuestaScript rs = null;
        final RVersionTramitePropiedades propiedadesTramite = pDatosSesion
                .getDefinicionTramite().getDefinicionVersion().getPropiedades();
        if (UtilsSTG.existeScript(
                propiedadesTramite.getScriptParametrosIniciales())) {
            final RScript scriptParam = propiedadesTramite
            ...
            rs = this.scriptFlujo.executeScriptFlujo(
                    TypeScriptFlujo.SCRIPT_PARAMETROS_INICIALES, null, script,
                    variablesFlujo, null, null, codigosError,
                    pDatosSesion.getDefinicionTramite());
            ...
            if (rs.isError()) {
                ...
                // la sesion
                final ResParametrosIniciales rp = (ResParametrosIniciales) rs
                        .getResultado();
                for (final String codParam : rp.getParametros().keySet()) {
                    pDatosSesion.getDatosTramite().getParametrosInicio()
                            .put(codParam, rp.getParametros().get(codParam));
                }
            }
        }
    }
    ...
    public void cancelarTramite(final DatosSesionTramitacion pDatosSesion) {
        ...
        pDatosSesion.getDatosTramite()
                .setEstadoTramite(TypeEstadoTramite.FINALIZADO);
    }

    @Override
    public void calcularMetodosLogin(
            final DatosSesionTramitacion pDatosSesion) {
     ...
     */
    private ControladorPaso getControladorPaso(
            final DatosSesionTramitacion pDatosSesion, final String idPaso) {
        ...
        if (cp == null) {
            throw new TipoNoControladoException(
                    "Tipo de paso no controlado para paso con id: " + idPaso);
        }
     ...
     */
    private String ejecutarScriptNavegacion(
            final DatosSesionTramitacion pDatosSesion, final String pIdPaso,
            final RPasoTramitacion defPasoActual) {
        String idPasoSiguiente = null;
        // Ejecutamos script de navegacion si procede
        if (!pIdPaso.equals(ConstantesFlujo.ID_PASO_GUARDAR)
                && !defPasoActual.isPasoFinal()
                && UtilsSTG.existeScript(defPasoActual.getScriptNavegacion())) {

            debug(pDatosSesion,
                    "Ejecucion script de navegacion paso " + pIdPaso);
            ...
            if (dp.getTipo() == TypePaso.CAPTURAR
                    || dp.getTipo() == TypePaso.RELLENAR
                    || dp.getTipo() == TypePaso.ACCION) {
                documentosPaso = this.getControladorPaso(pDatosSesion, pIdPaso)
                        .obtenerDocumentosPaso(dp,
                                pDatosSesion.getDefinicionTramite());
            }

            final RespuestaScript rs = this.scriptFlujo.executeScriptFlujo(
                    TypeScriptFlujo.SCRIPT_NAVEGACION_PASO, pIdPaso, script,
                    variablesFlujo, null, documentosPaso, codigosError,
                    pDatosSesion.getDefinicionTramite());
            ...
            if (rs.isError()) {
                ...
                if (StringUtils.isBlank(idPasoSiguiente)) {
                    ...
                    final RPasoTramitacion defPasoSiguiente = UtilsSTG
                            .devuelveDefinicionPaso(idPasoSiguiente,
                                    pDatosSesion.getDefinicionTramite());
                    if (defPasoSiguiente == null) {
                        throw new ErrorScriptException(
                                TypeScriptFlujo.SCRIPT_NAVEGACION_PASO.name(),
                                pDatosSesion.getDatosTramite()
                                        .getIdSesionTramitacion(),
                                pIdPaso,
                                "El script de navegacion no ha devuelto un id de paso correcto: "
                                        + idPasoSiguiente);
                    }
                }
            }
     ...
     *            Mensaje
     */
    private void debug(final DatosSesionTramitacion pDatosSesion,
            final String message) {
        if (UtilsSTG.isDebugEnabled(pDatosSesion.getDefinicionTramite())
