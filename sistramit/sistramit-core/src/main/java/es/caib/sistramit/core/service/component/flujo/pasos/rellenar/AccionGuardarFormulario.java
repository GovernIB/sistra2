...
public final class AccionGuardarFormulario implements AccionPaso {
    ...
    public RespuestaEjecutarAccionPaso ejecutarAccionPaso(DatosPaso pDatosPaso,
        ...
        final String idFormulario = (String) UtilsFlujo
                .recuperaParametroAccionPaso(pParametros, "idFormulario", true);
        final String ticket = (String) UtilsFlujo
                .recuperaParametroAccionPaso(pParametros, "ticket", false);
        TypeSiNo borrarOpcional = (TypeSiNo) UtilsFlujo
                .recuperaParametroAccionPaso(pParametros, "borrarOpcional",
                        false);
        ...
        if (borrarOpcional == TypeSiNo.NO) {
            ...
            if (StringUtils.isBlank(ticket)) {
                throw new ParametrosEntradaIncorrectosException(
                        "Falta especificar parametro ticket");
            }
            // Obtenemos datos finalizacion gestor formulario
            dff = obtenerDatosFinalizacionGestor(dipa, idFormulario, ticket,
                    pDefinicionTramite);
        } else {
        ...
        if (dff != null && dff.isCancelado()) {
            ...
            final RespuestaScript respuestaScriptPostGuardar = ejecutarScriptPostGuardar(
                    dipa, pDpp, idFormulario, dff, pDefinicionTramite,
                    pVariablesFlujo, borrarOpcional);

            // Actualizamos persistencia para despues recalcular detalle (los
            // datos de este formulario y los modificados en el script de
            // postguardar).
            if (borrarOpcional == TypeSiNo.NO) {
                actualizarPersistenciaGuardar(dipa, pDpp, pVariablesFlujo,
                        idFormulario, dff.getXml(), dff.getPdf(),
                        respuestaScriptPostGuardar);
            } else {
                // No borramos si hemos indicado error en el script
                if (respuestaScriptPostGuardar == null
                        || !respuestaScriptPostGuardar.isError()) {
                    actualizarPersistenciaCancelarFormulario(dipa, pDpp,
                            pVariablesFlujo, idFormulario,
                            respuestaScriptPostGuardar);
                }
            ...
            if (respuestaScriptPostGuardar != null
                ...
                rp.addParametroRetorno("mensajeIncorrecto",
                        respuestaScriptPostGuardar.getMensajeError());
            } else {
     ...
     */
    private DatosFinalizacionFormulario obtenerDatosFinalizacionGestor(
            final DatosInternosPasoRellenar pDipa, final String pIdFormulario,
        ...
        final RFormularioTramite formularioActualDef = UtilsSTG
                .devuelveDefinicionFormulario(pasoDef, pIdFormulario);
        DatosFinalizacionFormulario dff;

        if (formularioActualDef.isInterno()) {
            dff = controladorGestorFormulariosInterno
                    .obtenerDatosFinalizacionFormulario(pTicket);
        } else {
            dff = controladorGestorFormulariosExterno
                    .obtenerDatosFinalizacionFormulario(pTicket);
        }
        return dff;
    }
     ...
     *
     */
    private void validarGuardarFormulario(final DatosInternosPasoRellenar pDipa,
            final String pIdFormulario) {
        // Si el formulario es dependiente, no debería dejar guardarlo
        final Formulario formularioDetalle = ((DetallePasoRellenar) pDipa
                .getDetallePaso()).getFormulario(pIdFormulario);
        if (formularioDetalle == null) {
            throw new AccionPasoNoPermitidaException(
                    "El formulario no existe: " + pIdFormulario);
        }
        if (formularioDetalle
                .getObligatorio() == TypeObligatoriedad.DEPENDIENTE) {
            throw new AccionPasoNoPermitidaException(
                    "Un formulario dependiente no puede ser guardado");
        }
    }
     ...
     */
    private RespuestaScript ejecutarScriptPostGuardar(
            final DatosInternosPasoRellenar pDipa,
            final DatosPersistenciaPaso pDpp, final String pIdFormulario,
            final DatosFinalizacionFormulario pDff,
            final DefinicionTramiteSTG pDefinicionTramite,
        ...
        // ejecutar script
        if (UtilsSTG.existeScript(formularioDef.getScriptPostguardar())
                && (pBorrarOpcional == TypeSiNo.NO)) {

            // Obtenemos formularios completados anteriores al formulario
            // actual en este paso
            final List<DatosDocumento> documentosPaso = UtilsPasoRellenar
                    .obtenerDocumentosCompletados(pDipa, pDpp,
                            pDefinicionTramite,
            ...
            final Map<String, String> codigosError = UtilsSTG
                    .convertLiteralesToMap(formularioDef.getScriptPostguardar()
                            .getLiterales());
            rs = scriptFlujo.executeScriptFlujo(
                    TypeScriptFlujo.SCRIPT_POSTGUARDAR_FORMULARIO,
                    formularioDef.getIdentificador(),
                    formularioDef.getScriptPostguardar().getScript(),
                    pVariablesFlujo, null, documentosPaso, codigosError,
                    pDefinicionTramite);

            // Si se ha ejecutado correctamente comprobamos que los cambios
            // a realizar son sobre los demas formularios del paso
            if (!rs.isError()) {
                final ResModificacionFormularios resp = (ResModificacionFormularios) rs
                        .getResultado();
                // Verificamos que se pueden modificar los formularios que
                // se han modificado (pasaran a estado incorrecto)
                verificarFormulariosModificados(pDipa, pasoDef, formularioDef,
                        resp, pVariablesFlujo);
                // Verificamos que se pueden modificar los formularios que
                // se han marcado como incorrectos
                verificarFormulariosIncorrectos(pDipa, pasoDef, formularioDef,
                        resp, pVariablesFlujo);
            }
        }
    ...
    /**
     * Verificamos que se pueden modificar los formularios que se han marcado
     * como incorrectos.
     *
     * @param pDipa
     ...
     *            Variables de flujo
     */
    private void verificarFormulariosIncorrectos(
            final DatosInternosPasoRellenar pDipa,
            final RPasoTramitacionRellenar pPasoDef,
            final RFormularioTramite pFormularioDef,
            final ResModificacionFormularios pRespModifForms,
            final VariablesFlujo pVariablesFlujo) {
        for (final String idFormModif : pRespModifForms
                .getFormulariosIncorrectos()) {
            if (idFormModif.equals(pFormularioDef.getIdentificador())) {
                throw new ErrorScriptException(
                        TypeScriptFlujo.SCRIPT_POSTGUARDAR_FORMULARIO.name(),
                        pVariablesFlujo.getIdSesionTramitacion(),
                        pFormularioDef.getIdentificador(),
                        "Se ha intentado modificar estado del formulario actual");
            }
            if (((DetallePasoRellenar) pDipa.getDetallePaso())
                    .getFormulario(idFormModif) == null) {
                throw new ErrorScriptException(
                        TypeScriptFlujo.SCRIPT_POSTGUARDAR_FORMULARIO.name(),
                        pVariablesFlujo.getIdSesionTramitacion(),
                        pFormularioDef.getIdentificador(),
                        "Se ha intentado modificar estado de formulario de otro paso ("
                                + idFormModif + ")");
            }
            if (!esFormularioPosterior(pFormularioDef.getIdentificador(),
                    idFormModif, pPasoDef)) {
                throw new ErrorScriptException(
                        TypeScriptFlujo.SCRIPT_POSTGUARDAR_FORMULARIO.name(),
                        pVariablesFlujo.getIdSesionTramitacion(),
                        pFormularioDef.getIdentificador(),
                        "Se ha intentado modificar estado de formulario de un formulario anterior ("
                                + idFormModif + ")");
            }
        }
    }
    ...
    /**
     * Verifica que el formulario a modificar es posterior al formulario que se
     * esta guardando.
     *
     ...
     */
    private boolean esFormularioPosterior(final String pIdFormGuardar,
            final String pIdFormModif,
            final RPasoTramitacionRellenar pPasoDef) {
    ...
    /**
     * Verificamos que se pueden modificar los formularios que se han
     * modificado.
     *
     * @param pDipa
     *            Datos internos del paso
     ...
     *            Variables de flujo
     */
    private void verificarFormulariosModificados(
            final DatosInternosPasoRellenar pDipa,
            final RPasoTramitacionRellenar pPasoDef,
            final RFormularioTramite pFormularioDef,
            final ResModificacionFormularios pRespModifForms,
        ...
        for (final String idFormModif : pRespModifForms
            ...
            if (((DetallePasoRellenar) pDipa.getDetallePaso())
                ...
                throw new ErrorScriptException(
                        ...
                        pVariablesFlujo.getIdSesionTramitacion(),
                        pFormularioDef.getIdentificador(),
                        "Se ha intentado modificar el formulario de otro paso ("
                                + idFormModif + ")");
            }
            if (!esFormularioPosterior(pFormularioDef.getIdentificador(),
                    idFormModif, pPasoDef)) {
                throw new ErrorScriptException(
                        ...
                        pVariablesFlujo.getIdSesionTramitacion(),
                        pFormularioDef.getIdentificador(),
                        "Se ha intentado modificar un formulario anterior ("
                                + idFormModif + ")");
            }
        }
    }
     ...
     *
     * Deberá actualizar los datos del formulario que se guarda y los
     * datos/estado de los formularios que se modifiquen en el script de
     * postguardar.
     *
     * @param pDipa
     *            Datos internos paso
     ...
     */
    private void actualizarPersistenciaGuardar(
            final DatosInternosPasoRellenar pDipa,
            final DatosPersistenciaPaso pDpp,
            final VariablesFlujo pVariablesFlujo, final String idFormulario,
            final byte[] xml, final byte[] pdf, final RespuestaScript rs) {

        // Actualizamos datos formulario actual
        actualizarFormularioActual(pDipa, pDpp, idFormulario, xml, pdf, rs,
                pVariablesFlujo);

        // Actualizamos formularios modificados en post guardar
        if (rs != null && !rs.isError()) {
            actualizarFormulariosPostGuardar(pDipa, pDpp, idFormulario, rs,
                    pVariablesFlujo);
        }
     ...
     *            Variables de flujo
     */
    private void actualizarFormularioActual(
            final DatosInternosPasoRellenar pDipa,
            final DatosPersistenciaPaso pDpp, final String idFormulario,
            final byte[] xml, final byte[] pdf, final RespuestaScript rs,
            final VariablesFlujo pVariablesFlujo) {
        ...
        // postguardar
        if (rs != null && !rs.isError()) {
            final ResModificacionFormularios rsp = (ResModificacionFormularios) rs
                    .getResultado();
            if (rsp.getFormulariosModificados().contains(idFormulario)) {
                final ValoresFormulario valoresFormulario = new ValoresFormulario(
                        xml);
                final List<ValorCampo> valoresModificados = rsp
                        .getDatosModificadosFormulario(idFormulario);
        ...
        final DocumentoPasoPersistencia docPaso = pDpp
        ...
        referenciaXML = dao.insertarFicheroPersistencia(idFormulario + ".xml",
                xmlFormActual, pVariablesFlujo.getIdSesionTramitacion());
        if (pdf != null) {
            referenciaPDF = dao.insertarFicheroPersistencia(
                    idFormulario + ".pdf", pdf,
                    pVariablesFlujo.getIdSesionTramitacion());
        }
     ...
     *            Variables flujo
     */
    private void actualizarFormulariosPostGuardar(
            final DatosInternosPasoRellenar pDipa,
            final DatosPersistenciaPaso pDpp, final String pIdFormularioActual,
            final RespuestaScript rs, final VariablesFlujo pVariablesFlujo) {
        ...
        for (final String idFormModif : rsp.getFormulariosModificados()) {
            ...
            final ValoresFormulario vf = pDipa
                    .getValoresFormulario(idFormModif);
            vf.modificarValoresCampos(
                    rsp.getDatosModificadosFormulario(idFormModif));
            referenciaXML = dao.insertarFicheroPersistencia(
                    idFormModif + ".xml", vf.getXml(),
                    pVariablesFlujo.getIdSesionTramitacion());

            // Establecemos nuevos datos
            if (docPasoModif
                    .getEstado() == TypeEstadoDocumento.RELLENADO_CORRECTAMENTE) {
                docPasoModif.setEstado(
     ...
     *
     */
    private void validarCancelarFormulario(
            final DatosInternosPasoRellenar pDipa, final String pIdFormulario) {
        final Formulario formularioDetalle = ((DetallePasoRellenar) pDipa
                .getDetallePaso()).getFormulario(pIdFormulario);
        if (formularioDetalle == null) {
        ...
        if (formularioDetalle.getObligatorio() != TypeObligatoriedad.OPCIONAL) {
            throw new AccionPasoNoPermitidaException(
                    "Solo puede ser cancelado un formulario opcional");
        }
        if (formularioDetalle
                .getRellenado() == TypeEstadoDocumento.SIN_RELLENAR) {
            throw new AccionPasoNoPermitidaException(
    ...
    /**
     * Actualiza datos de persistencia en el paso de cancelar formulario
     * opcional.
     *
     * @param pDipa
     ...
     */
    private void actualizarPersistenciaCancelarFormulario(
            final DatosInternosPasoRellenar pDipa,
            final DatosPersistenciaPaso pDpp,
            final VariablesFlujo pVariablesFlujo, final String pIdFormulario,
        ...
        if (pRespuestaScriptPostGuardar != null
                && !pRespuestaScriptPostGuardar.isError()) {
            actualizarFormulariosPostGuardar(pDipa, pDpp, pIdFormulario,
                    pRespuestaScriptPostGuardar, pVariablesFlujo);
        }
    }
}