...
public final class ControladorFlujoTramitacionImpl
        implements ControladorFlujoTramitacion, Serializable {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(ControladorFlujoTramitacionImpl.class);

    @Override
    public ResultadoIrAPaso iniciarTramite(
            final DatosSesionTramitacion pDatosSesion) {

        debug(pDatosSesion, "Iniciar tramite");

        // Obtenemos reglas del flujo
        final ReglasFlujo reglas = obtenerReglasFlujo(
                pDatosSesion.getDatosTramite().getTipoFlujo());

        // Generamos contexto ejecucion reglas
        final ContextoReglaTramitacion ctx = generarContextoReglaTramitacion(
                pDatosSesion);
        ...
        if (!ejecutarReglas(ctx, reglas, TypeFaseEjecucion.PRE_INICIO, null)) {
            throw new ErrorConfiguracionException(
                    "Error aplicando regla de tramitación al iniciar trámite");
        }

        // Se establece primer paso como actual y se va a dicho paso
        final String idPrimerPaso = pDatosSesion.getDatosTramite()
                .getDatosPasos().get(0).getIdPaso();
    ...
    @Override
    public ResultadoIrAPaso cargarTramite(
            final DatosSesionTramitacion pDatosSesion) {
        ...
        if (!ejecutarReglas(ctx, reglas, TypeFaseEjecucion.PRE_CARGA, null)) {
            throw new ErrorConfiguracionException(
                    "Error aplicando regla de tramitación al cargar trámite");
        }

        // Vamos a siguiente paso
        final ResultadoIrAPaso rp = this.irAPaso(pDatosSesion,
    ...
    @Override
    public ResultadoIrAPaso irAPaso(final DatosSesionTramitacion pDatosSesion,
            final String pIdPaso) {

        debug(pDatosSesion, "Ir a paso " + pIdPaso);

        // Obtenemos estado inicial
        final TypeEstadoTramite estadoInicial = pDatosSesion.getDatosTramite()
                .getEstadoTramite();

        // Recuperamos definicion paso
        final RPasoTramitacion pasoDef = UtilsSTG.devuelveDefinicionPaso(
                pIdPaso, pDatosSesion.getDefinicionTramite());
        if (pasoDef == null
                && !(ConstantesFlujo.ID_PASO_GUARDAR.equals(pIdPaso))) {
            throw new AccionPasoNoPermitidaException(
                    "Paso no existe: " + pIdPaso);
        }
        final TypePaso tipoPaso = TypePaso.fromString(pasoDef.getTipo());
        if (tipoPaso == null) {
            throw new TipoNoControladoException("Tipo de paso no controlado: "
                    + pasoDef.getIdentificador());
        }

        // Control bucle infinito si es un paso de tipo accion que vuelve a si
        // mismo
        if (tipoPaso == TypePaso.ACCION && pIdPaso
                .equals(pDatosSesion.getDatosTramite().getIdPasoActual())) {
            throw new ErrorConfiguracionException(
                    "Un paso accion no puede volver sobre si mismo");
        }
        ...
        // actual)
        if (ejecutarReglas(ctx, reglas, TypeFaseEjecucion.PRE_NAVEGACION,
                vars)) {
            ...
            // Ejecutamos reglas post
            if (!ejecutarReglas(ctx, reglas, TypeFaseEjecucion.POST_NAVEGACION,
                    vars)) {
                throw new ErrorConfiguracionException(
                        "Error aplicando regla de tramitación al ir a paso");
            }

            // Si el paso es de tipo accion ejecutamos automaticamente la accion
            if (tipoPaso == TypePaso.ACCION) {
                this.accionPaso(pDatosSesion, pIdPaso,
                        TypeAccionPasoAccion.EJECUTAR_ACCION, null);
            }
        }
        ...
        final TypeEstadoTramite estadoFinal = pDatosSesion.getDatosTramite()
                .getEstadoTramite();
        if (estadoFinal == TypeEstadoTramite.FINALIZADO
                && estadoInicial != TypeEstadoTramite.FINALIZADO) {
    ...
    @Override
    public ResultadoAccionPaso accionPaso(
        ...
        if (!ejecutarReglas(ctx, reglas, TypeFaseEjecucion.PRE_ACCION, vars)) {
            throw new ErrorConfiguracionException(
                    "Error aplicando regla de tramitación al ejecutar acción: "
                            + accionPaso.name() + " en paso " + idPaso);
        }

        // Invocamos paso
        final RespuestaAccionPaso rp = modificacionesFlujo.invocarAccionPaso(
                pDatosSesion, idPaso, accionPaso, parametros);
        ...
        if (siguientePost
                || accionPaso == TypeAccionPasoAccion.EJECUTAR_ACCION) {
            debug(pDatosSesion, "Navegacion automatica a "
                    + pDatosSesion.getDatosTramite().getIdPasoSiguiente());
            irAPaso(pDatosSesion,
                    pDatosSesion.getDatosTramite().getIdPasoSiguiente());
        }
        ...
        res.setFinalizadoTrasAccion((estadoFinal == TypeEstadoTramite.FINALIZADO
                && estadoInicial != TypeEstadoTramite.FINALIZADO));
        return res;
    }
    ...
    public void cancelarTramite(final DatosSesionTramitacion pDatosSesion) {
        ...
        if (!ejecutarReglas(ctx, reglas, TypeFaseEjecucion.PRE_CANCELAR,
                null)) {
            throw new CancelacionNoPermitidaException(
    ...
    @Override
    public DetallePasos detallePasos(
            final DatosSesionTramitacion pDatosSesion) {
        ...
        // - Estado tramite
        detallePasos
                .setEstado(pDatosSesion.getDatosTramite().getEstadoTramite());

        // - Lista de pasos
        final List<PasoLista> listaPasos = detalleTramiteCalcularPasosLista(
                pDatosSesion);
        detallePasos.setPasos(listaPasos);

        // - Paso actual
        final DetallePaso dtpa = modificacionesFlujo.obtenerDetallePaso(
                pDatosSesion, pDatosSesion.getDatosTramite().getIdPasoActual());
        detallePasos.setActual(dtpa);

        // - Paso anterior y siguiente (calculamos a partir de la lista de
        // pasos)
        final NavegacionPaso np = detalleTramiteCalcularNavegacionPaso(
                pDatosSesion);
     ...
     *            Datos de sesión.
     */
    private void personalizacionTramite(
            final DatosSesionTramitacion pDatosSesion) {

        // Ejecutamos script de personalizacion
        final RespuestaScript rs = modificacionesFlujo
                .ejecutarScriptPersonalizacion(pDatosSesion);
        ...
        if (rs != null) {
            if (rs.isError()) {
                ...
                final ResPersonalizacionTramite rp = (ResPersonalizacionTramite) rs
                        .getResultado();
    ...
    /**
     * Realiza control de acceso al trámite: activo, plazos y filtro de
     * certificados y grupos. En caso de no permitir el acceso se generará una
     * excepción AccesoNoPermitidoException.
     *
     * @param pDatosSesion
    ...
    private void controlAcceso(final DatosSesionTramitacion pDatosSesion) {
        ...
        if (!controlTipoAutenticacion(pDatosSesion)) {
            mensaje = literales.getLiteral(Literales.FLUJO,
                    "acceso.tipoAutenticacionNoPermitida",
        ...
        final RVersionTramiteControlAcceso propsControlAcceso = pDatosSesion
                .getDefinicionTramite().getDefinicionVersion()
                .getControlAcceso();
     ...
     *            Datos Sesion
     */
    private void controlBloqueosAvisosPlataforma(
            final DatosSesionTramitacion pDatosSesion) {
        ...
        final List<AvisoPlataforma> avisos = UtilsSTG.obtenerAvisosTramite(
                pDatosSesion.getDefinicionTramite(), avisosPlataforma,
                pDatosSesion.getDatosTramite().getIdioma(), true);
     ...
     */
    private ListaPropiedades generarDetallesExcepcionControlAcceso(
            final DatosSesionTramitacion pDatosSesion) {
        final ListaPropiedades detalles = new ListaPropiedades();
        detalles.addPropiedad("Tramite Id",
                pDatosSesion.getDatosTramite().getIdTramite());
        detalles.addPropiedad("Tramite Vers", Integer
                .toString(pDatosSesion.getDatosTramite().getVersionTramite()));
        return detalles;
    }
     ...
     *            Datos sesion
     */
    protected void controlPlazos(final RVersionTramiteControlAcceso wa,
            final Date ahora, final DatosSesionTramitacion pDatosSesion) {

        String mensaje;

        // Control de plazos tramite
        if (!UtilsFlujo.estaDentroPlazo(ahora,
                pDatosSesion.getDatosTramite().getPlazoInicio(),
                pDatosSesion.getDatosTramite().getPlazoFin())
                || !pDatosSesion.getDatosTramite().isVigente()) {
        ...
        final Date fechaCaducidad = pDatosSesion.getDatosTramite()
                .getFechaCaducidad();
        if (fechaCaducidad != null
                && UtilsFlujo.obtenerUltimaHora(fechaCaducidad)
                        .before(UtilsFlujo.obtenerPrimeraHora(new Date()))) {
            throw new TramiteCaducadoException(
                    pDatosSesion.getDatosTramite().getIdSesionTramitacion());
        }
    }
     ...
     * @return booleano
     */
    private boolean controlTipoAutenticacion(
            final DatosSesionTramitacion pDatosSesion) {
        boolean permitido = true;
        final TypeAutenticacion nivelAutenticacion = pDatosSesion
                .getDatosTramite().getNivelAutenticacion();
        permitido = pDatosSesion.getDatosTramite().getMetodosLogin()
                .contains(nivelAutenticacion);
     ...
     *            Id paso
     */
    private void controlAccesoPaso(final DatosSesionTramitacion pDatosSesion,
            final String pIdPaso) {
        ...
        if (dp == null) {
            throw new AccionPasoNoPermitidaException(
                    "Paso " + pIdPaso + " no existe. El paso actual es: "
                            + pDatosSesion.getDatosTramite().getIdPasoActual());
        }
        if (!dp.getIdPaso()
                .equals(pDatosSesion.getDatosTramite().getIdPasoActual())) {
            throw new AccionPasoNoPermitidaException("El paso " + pIdPaso
                    + " no es el actual. El paso actual es: "
                    + pDatosSesion.getDatosTramite().getIdPasoActual());
        }
        ...
        if (dp.getEstado() == TypeEstadoPaso.NO_INICIALIZADO
            ...
            throw new AccionPasoNoPermitidaException("El paso " + pIdPaso
                    + " no esta inicializado o esta pendiente de revisar");
        }
    }
     ...
     */
    private List<PasoLista> detalleTramiteCalcularPasosLista(
            final DatosSesionTramitacion pDatosSesion) {
        ...
        for (final DatosPaso dp : pDatosSesion.getDatosTramite()
            ...
            final boolean accesible = pDatosSesion.getDatosTramite()
                    .getAccesibilidadPaso(dp.getIdPaso());
     ...
     */
    private NavegacionPaso detalleTramiteCalcularNavegacionPaso(
            final DatosSesionTramitacion pDatosSesion) {
        final NavegacionPaso np = new NavegacionPaso();

        final int indicePasoActual = pDatosSesion.getDatosTramite()
                .getIndiceDatosPasoActual();
        final DatosPaso datosPasoActual = pDatosSesion.getDatosTramite()
                .getDatosPasoActual();

        // Paso anterior
        if (indicePasoActual > 0) {
            final DatosPaso datosPasoAnterior = pDatosSesion.getDatosTramite()
                    .getDatosPasos()
                    .get(indicePasoActual - ConstantesNumero.N1);
            if (pDatosSesion.getDatosTramite()
                    .getAccesibilidadPaso(datosPasoAnterior.getIdPaso())) {
        ...
        if (datosPasoActual.getEstado() == TypeEstadoPaso.COMPLETADO
                && !datosPasoActual.isPasoFinal()) {
            final String idPasoSiguiente = pDatosSesion.getDatosTramite()
                    .getIdPasoSiguiente();
     ...
     * @return Reglas flujo
     */
    private ReglasFlujo obtenerReglasFlujo(
            final TypeFlujoTramitacion tipoFlujo) {

        ReglasFlujo reglasFlujo = null;
        switch (tipoFlujo) {
        case NORMALIZADO:
            reglasFlujo = (ReglasFlujo) ApplicationContextProvider
                    .getApplicationContext().getBean("reglasFlujoNormalizado");
            break;
        case PERSONALIZADO:
            reglasFlujo = (ReglasFlujo) ApplicationContextProvider
        ...
        default:
            throw new TipoNoControladoException(
                    "Tipo de flujo " + tipoFlujo.name() + " no controlado");
        }
     ...
     */
    private ContextoReglaTramitacion generarContextoReglaTramitacion(
            final DatosSesionTramitacion pDatosSesion) {
        final ContextoReglaTramitacion ctx = new ContextoReglaTramitacion(
                pDatosSesion);
        return ctx;
    }
     ...
     */
    private boolean ejecutarReglas(final ContextoReglaTramitacion ctx,
            final ReglasFlujo reglas, final TypeFaseEjecucion fase,
            final Object[] vars) {
        boolean res = true;
        final List<ReglaTramitacion> reglasPre = reglas
                .getReglasTramitacion(fase);
        if (reglasPre != null) {
