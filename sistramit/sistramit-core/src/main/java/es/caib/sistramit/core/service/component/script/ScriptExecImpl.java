...
public final class ScriptExecImpl implements ScriptExec {
    ...
    @Override
    public RespuestaScript executeScriptFlujo(final TypeScriptFlujo pTipoScript,
            final String pIdElemento, final String pScript,
            final VariablesFlujo pVariablesFlujo,
            final Map<String, Object> pVariablesScript,
        ...
        // Generamos plugins
        final List<PluginScript> plugins = generarPluginsFlujo(pTipoScript,
                pIdElemento, pVariablesFlujo, pVariablesScript, pDocumentosPaso,
                pDefinicionTramite, codigosError);

        // Ejecutamos script
        final String idSesion = pVariablesFlujo.getIdSesionTramitacion();
        final Object result = ejecutarScript(pTipoScript, idSesion, pIdElemento,
                pScript, plugins);

        // Retornamos resultado
        return generarRespuestaScriptFlujo(pTipoScript, codigosError, plugins,
                result);
    }
     ...
     */
    private Object ejecutarScript(final TypeScript pTipoScript,
            final String idSesion, final String pIdElemento,
            final String pScript, final List<PluginScript> plugins) {
        Object result = null;
        try {
            // Preparamos scripts para que valgan los returns
            final StringBuilder sb = new StringBuilder(
                    pScript.length() + ConstantesNumero.N60);
            sb.append("function ejecutarScript() { ").append(pScript)
                    .append("\n}; ejecutarScript();");
            final ScriptEngine jsEngine = engineManager
            ...
            throw new EngineScriptException(pTipoScript.name(), idSesion,
                    pIdElemento, ex.getLineNumber(),
     ...
     */
    private RespuestaScript generarRespuestaScriptFlujo(
            final TypeScriptFlujo pTipoScript,
            final Map<String, String> codigosError,
            final List<PluginScript> plugins, final Object result) {
        ...
        if (plgError.isExisteError()) {
            ...
            if (StringUtils.isNotBlank(plgError.getCodigoMensajeError())) {
                respuestaScript.setMensajeError(ScriptUtils.calculaMensajeError(
                        codigosError, plgError.getCodigoMensajeError(),
                        plgError.getParametrosMensajeError()));
            } else if (StringUtils
                    .isNotBlank(plgError.getTextoMensajeError())) {
                respuestaScript
                        .setMensajeError(plgError.getTextoMensajeError());
            } else {
                respuestaScript
                        .setMensajeError("No se ha establecido mensaje error");
            }
        }
        // - En funcion del plugin vemos si debemos retornar el objeto o bien un
        // plugin de tipo resultado
        final Object resScr = obtenerResultadoScript(pTipoScript, plugins,
                result);
     ...
     *            Codigos de error
     */
    private void generarMensajeAviso(final RespuestaScript respuestaScript,
            final List<PluginScript> plugins,
            final Map<String, String> codigosError) {
        ...
        if (plgAviso != null && plgAviso.isExisteAviso()) {
            ...
            final String textoMensajeAviso = calcularMensaje(
                    plgAviso.getCodigoMensajeAviso(),
                    plgAviso.getParametrosMensajeAviso(),
                    plgAviso.getTextoMensajeAviso(), codigosError);
     ...
     * @return resultado script
     */
    private Object obtenerResultadoScript(final TypeScriptFlujo pTipoScript,
            final List<PluginScript> plugins, final Object result) {
        Object res = null;
     ...
     */
    private List<PluginScript> generarPluginsFlujo(
            final TypeScriptFlujo pTipoScript, final String pIdElemento,
            final VariablesFlujo pVariablesFlujo,
            final Map<String, Object> pVariablesScript,
            final List<DatosDocumento> pDocumentosPaso,
            final DefinicionTramiteSTG pDefinicionTramite,
        ...
        // - Plugin log
        plugins.add(new PlgLog(pVariablesFlujo.getIdSesionTramitacion(),
                pTipoScript, pIdElemento, pVariablesFlujo.isDebugEnabled()));
        // - Plugin dominios
        plugins.add(new PlgDominios(pDefinicionTramite));
        // - Plugin formularios
        ...
        case SCRIPT_VALIDAR_ANEXO:
            final String nomFichero = (String) pVariablesScript
                    .get("nombreFichero");
            final byte[] datosFichero = (byte[]) pVariablesScript
                    .get("datosFichero");
     ...
     * @return Plugin
     */
    private PluginScript getPlugin(final List<PluginScript> plugins,
            final String name) {
     ...
     * @return mensaje
     */
    private String calcularMensaje(final String codigoMensajeError,
            final List<String> parametrosMensajeError,
            final String textoMensajeErrorParticularizado,
            final Map<String, String> pCodigosError) {
        String textoMensajeError;
        if (StringUtils.isNotBlank(codigoMensajeError)) {
            textoMensajeError = ScriptUtils.calculaMensajeError(pCodigosError,
                    codigoMensajeError, parametrosMensajeError);
     ...
     */
    private PlgFormularios crearPluginFormularios(
            final VariablesFlujo pVariablesFlujo,
            final List<DatosDocumento> pDocumentosPaso) {
