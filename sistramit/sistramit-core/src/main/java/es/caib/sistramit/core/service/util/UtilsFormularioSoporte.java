...
public final class UtilsFormularioSoporte {
     ...
     * @return asunto
     */
    public static String obtenerAsuntoFormularioSoporte(Literales literales,
            DatosSesionTramitacion sesion) {
        final String asunto = literales.getLiteral(
                Literales.SOPORTE_INCIDENCIAS, "asunto",
     ...
     * @return
     */
    public static List<String> obtenerDestinatariosFormularioSoporte(
            DatosFormularioSoporte datosFormSoporte,
            final RConfiguracionEntidad entidad,
            DatosSesionTramitacion datosSesion) {

        final ROpcionFormularioSoporte opc = obtenerOpcionFormularioSoporte(
                datosFormSoporte.getProblemaTipo(), entidad);
        ...
        if ("R".equals(opc.getDestinatario())) {
            ...
            String emailDest = datosSesion.getDatosTramite()
                    .getDefinicionTramiteCP().getEmailSoporte();
            ...
            if (!ValidacionesTipo.getInstance().esEmail(emailDest)) {
                throw new ErrorFormularioSoporteException(
                        "Email no es válido: " + emailDest);
            }
            destinatarios.add(emailDest);
        } else if ("L".equals(opc.getDestinatario())) {
            // Lista destinatarios
            ...
            throw new ErrorFormularioSoporteException(
                    "Tipo destinatario no soportado: " + opc.getDestinatario());
        }
     ...
     * @return opcion
     */
    private static ROpcionFormularioSoporte obtenerOpcionFormularioSoporte(
            String problemaTipo, final RConfiguracionEntidad entidad) {
        ROpcionFormularioSoporte opc = null;
        if (entidad.getAyudaFormulario() != null) {
            for (final ROpcionFormularioSoporte o : entidad
                    .getAyudaFormulario()) {
        ...
        if (opc == null) {
            throw new ErrorFormularioSoporteException(
                    "No existe opcion con codigo: " + problemaTipo);
        }
        return opc;
    }
     ...
     * @return mensaje (html)
     */
    public static String construyeMensajeSoporteIncidencias(Literales literales,
            DatosFormularioSoporte datosFormSoporte,
            RConfiguracionEntidad entidad, DatosSesionTramitacion datosSesion) {
        ...
        String listaCampos = "";
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "tramiteDesc"),
                datosSesion.getDatosTramite().getTituloTramite());
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "tramiteId"),
                datosSesion.getDatosTramite().getIdTramite());
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "fechaCreacion"),
                formateaFecha(datosSesion.getDatosTramite().getFechaInicio()));
        if (datosSesion.getDatosTramite()
                .getNivelAutenticacion() == TypeAutenticacion.ANONIMO) {
            listaCampos += addParameterMensaje(
                    getLiteral(literales, idioma, "idPersistencia"),
                    datosSesion.getDatosTramite().getIdSesionTramitacion());
        }
        listaCampos += addParameterMensaje(getLiteral(literales, idioma, "nif"),
                datosFormSoporte.getNif());
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "nombre"),
                datosFormSoporte.getNombre());
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "telefono"),
                datosFormSoporte.getTelefono());
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "email"),
                datosFormSoporte.getEmail());
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "problemaTipo"),
                UtilsSTG.obtenerLiteral(opc.getDescripcion(), idioma));
        listaCampos += addParameterMensaje(
                getLiteral(literales, idioma, "problemaDesc"),
                datosFormSoporte.getProblemaDesc());

        String mensaje = getPlantilla();
        mensaje = StringUtils.replace(mensaje, "[#TITULO#]", StringEscapeUtils
                .escapeHtml4(getLiteral(literales, idioma, "titulo")));
     ...
     * @return literal
     */
    public static String getLiteral(Literales literales, final String idioma,
            String campo) {
        return literales.getLiteral(Literales.SOPORTE_INCIDENCIAS, campo,
                idioma);
    }
     ...
     * @return parametro
     */
    private static String addParameterMensaje(String paramDesc,
            String paramValue) {
        String res = "";
        if (StringUtils.isNotBlank(paramValue)) {
            res = "\n<tr><th>" + StringEscapeUtils.escapeHtml4(paramDesc)
                    + "</th><td>" + StringEscapeUtils.escapeHtml4(paramValue)
                    + "</td></tr>";
        }
        return res;
    }
    ...
    private static String getPlantilla() {
        try {
            ...
            throw new ErrorFormularioSoporteException(
                    "No se ha podido cargar la plantilla para envio aviso", ex);
        }
    }
     ...
     * @return string
     */
    private static String formateaFecha(Date fecha) {
