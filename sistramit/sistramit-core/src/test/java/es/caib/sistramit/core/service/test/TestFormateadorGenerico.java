...
public class TestFormateadorGenerico {
	public static void main(final String args[]) throws Exception {
		...
		// La siguiente linea carga un fichero de propiedades ya con tres valores
		// generico.
		properties.load(inputpropiedad);

		// Si se desea cambiar la url de prueba,
		// properties.put(FormateadorGenerico.IMAGEN_URL,
		// "");

		// Si se desea cambiar el tipo de visualizacion.
		// properties.put(FormateadorGenerico.LISTADO_VISUALIZACION,
		// TipoVisualizacionListado.SUFIJO_PARENTESIS);

		// Si se desea quitar/mostrar los mensajes de aviso, rellenar el siguiente dato.
		// properties.put(FormateadorGenerico.AVISO_MOSTRAR, true);

		final FormateadorGenerico formateador = new FormateadorGenerico();
		final byte[] resultado = formateador.formatear(UtilsFormulario.valoresToXml(formulario),
				SerializationUtils.serialize(properties), "es", formularioInterno);
