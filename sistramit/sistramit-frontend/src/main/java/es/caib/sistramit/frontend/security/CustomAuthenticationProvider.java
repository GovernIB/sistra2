...
public class CustomAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(final Authentication authentication) {
		...
		final SesionHttp sesionHttp = (SesionHttp) ApplicationContextProvider.getApplicationContext()
				.getBean("sesionHttp");
		...
		try {
			...
			if (ConstantesSeguridad.ANONIMO_USER.equals(usuario)) {
				...
				usuarioAutenticadoInfo = securityService.validarUsuarioAnonimo(sesionInfo);
			} else if (ConstantesSeguridad.TICKET_USER_CARPETA.equals(usuario)) {
				// Autenticacion por ticket
				debug("Autenticacion desde Carpeta: " + passwd);
				// Validamos ticket
				usuarioAutenticadoInfo = securityService.validarTicketCarpetaCiudadana(sesionInfo, passwd);
			} else if (ConstantesSeguridad.TICKET_USER_CLAVE.equals(usuario)) {
				// Autenticacion por ticket
				debug("Autenticacion por Clave: " + passwd);
				// Validamos ticket
				usuarioAutenticadoInfo = securityService.validarTicketClave(sesionInfo, passwd);
			} else if (ConstantesSeguridad.TICKET_USER_GF.equals(usuario)) {
				// Autenticacion por ticket
				debug("Autenticacion desde Gestor Formularios: " + passwd);
				// Validamos ticket
				usuarioAutenticadoInfo = securityService.validarTicketGestorFormularios(sesionInfo, passwd);
			} else if (ConstantesSeguridad.TICKET_USER_PAGO.equals(usuario)) {
				// Autenticacion por ticket
				debug("Autenticacion desde Pasarela Pagos: " + passwd);
				// Validamos ticket
				usuarioAutenticadoInfo = securityService.validarTicketPasarelaPagos(sesionInfo, passwd);
			} else {
				throw new BadCredentialsException("Usuario no valido: " + usuario);
			}
		...
		debug("Retornando usuario autenticado: " + usuarioAutenticadoInfo.getUsername());
		final UsuarioAutenticado usu = new UsuarioAutenticado();
		usu.setUsuario(usuarioAutenticadoInfo);
		return new UsernamePasswordAuthenticationToken(usu, authentication.getCredentials(), usu.getAuthorities());
