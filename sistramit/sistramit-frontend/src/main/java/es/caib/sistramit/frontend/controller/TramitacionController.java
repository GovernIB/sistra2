...
public abstract class TramitacionController {
	...
	public final ModelAndView handleServiceException(final Exception pex, final HttpServletRequest request) {
		...
		// controlada de front
		Exception ex = pex;
		if (!(pex instanceof ServiceException) && !(pex instanceof ErrorFrontException)) {
	...
	protected String getUrlAsistente() {
		// TODO Ver si cachear
		return systemService.obtenerPropiedadConfiguracion(TypePropiedadConfiguracion.URL_SISTRAMIT);
	}
	...
	protected final MensajeAsistente generarMensajeErrorAsistente(final String literalTitulo,
		...
		final String textoDetalleError = getLiteralesFront().getLiteralFront(LiteralesFront.ASISTENTE, "detalleError",
				getIdioma());
		final String tituloMsg = getLiteralesFront().getLiteralFront(LiteralesFront.ASISTENTE, literalTitulo,
				getIdioma());

		final String textoMsg = getLiteralesFront().getLiteralFront(LiteralesFront.ASISTENTE, literalMensaje,
				getIdioma());
		String detalleError = "";
