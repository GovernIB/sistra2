...
public final class DetallePasoRellenar extends DetallePaso {
	...
	public String print() {
		...
		final StringBuffer strb = new StringBuffer(ConstantesNumero.N8 * ConstantesNumero.N1024);
		...
		for (final Formulario formulario : this.getFormularios()) {
			...
			strb.append(identForms).append(ident).append(" Id:" + formulario.getId() + "\n");
			strb.append(identForms).append(ident).append(" Titulo:" + formulario.getTitulo() + "\n");

			strb.append(identForms).append(ident).append(" Firmar:" + formulario.getFirmar() + "\n");
			strb.append(identForms).append(ident).append(" Obligatorio:" + formulario.getObligatorio() + "\n");
			strb.append(identForms).append(ident).append(" Rellenado:" + formulario.getRellenado() + "\n");

			strb.append(identForms).append(ident).append(" Firmantes:\n");
			for (final Firmante firmante : formulario.getFirmantes()) {
				strb.append(identForms).append(ident).append(ident).append("   NIF:" + firmante.getNif() + "\n");
				strb.append(identForms).append(ident).append(ident).append("   nombre:" + firmante.getNombre() + "\n");
			}

			strb.append(identForms).append(ident).append(" Firmas:\n");
			for (final Firma firma : formulario.getFirmas()) {

				strb.append(identForms).append(ident).append(ident)
						.append("    Estado firma:" + firma.getEstadoFirma() + "\n");

				strb.append(identForms).append(ident).append(ident).append("    Rechazo:" + firma.getRechazo() + "\n");
				strb.append(identForms).append(ident).append(ident)
