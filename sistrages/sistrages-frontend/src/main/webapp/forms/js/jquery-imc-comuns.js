...
// appErrors
$.fn.appErrors = function(options) {
	var settings = $.extend({
		element: ""
	}, options);
	this.each(function(){
		var element = $(this),
			errors_el = $("#imc-forms-errors"),
			verifica = function() {
	...
	this.each(function(){
		var element = $(this),
			...
			inicia = function() {
				...
				var imc_finestra_H = imc_finestra.height(),
					imc_finestra_scroll_T = imc_finestra.scrollTop(),
					imc_document_H = imc_document.outerHeight(true),
					imc_document_W = imc_document.outerWidth(true);
				...
				destacat_el = $("<div>").addClass("imc-destacat").attr("id", "imc-destacat").append( destacat_el_T ).append( destacat_el_B ).append( destacat_el_L ).append( destacat_el_R ).appendTo( $("body") );
...
$.fn.ajuda = function(opcions) {
	...
	this.each(function(){
		var element = $(this),
			...
			onClick = function() {
				...
				if (imc_forms_ajuda.data("ajuda") === "off") {
					imc_forms_ajuda.removeClass("imc--desactivada");
					imc_forms_formulari.ajudaCamp();
					txtAjudaInfo = txtAjuda + " " + txtActivada;
					txtAjudaBoto = txtDesactiva;
					ajuda_data = "on";
				} else {
					imc_forms_ajuda.addClass("imc--desactivada");
					imc_forms_formulari.off('.ajudaCamp');
				...
				if (!Modernizr.csstransforms) {
					imc_forms_ajuda.fadeOut(200, function() {
						canvia();
						imc_forms_ajuda.fadeIn(200);
					});
				} else {
					canvia();
				}
			...
			canvia = function() {
				...
				imc_forms_ajuda
					.find("strong").text(txtAjudaInfo).end()
					.find("button").text(txtAjudaBoto).end()
					.data("ajuda", ajuda_data)
					.addClass("flip animated");
				
				var espera = window.setTimeout( function(){
						imc_forms_ajuda.removeClass("flip animated")
					},
					1300
				);
		...
		if (!imc_forms_ajuda.hasClass("imc--desactivada")) {
			imc_forms_formulari.ajudaCamp();
			imc_forms_ajuda.data("ajuda", "on");
		}
		// inicia
		imc_forms_ajuda.data("ajuda", ajuda_inicial);
