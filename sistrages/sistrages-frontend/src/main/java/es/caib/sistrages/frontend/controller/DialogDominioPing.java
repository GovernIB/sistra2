...
public class DialogDominioPing extends DialogControllerBase {
    ...
    public void init() {
        ...
        // quitar
        // el boton ping
        final boolean isTipoListaFija = dominio
                .getTipo() == TypeDominio.LISTA_FIJA;
        mostrarTablaParametro = !isTipoListaFija;
        mostrarTablaDatos = isTipoListaFija;

        // Si es tipo lista fija, rellenar los datos.
        if (isTipoListaFija && dominio.getListaFija() != null
                && !dominio.getListaFija().isEmpty()) {
        ...
        if (valoresDominio != null) {
            if (valoresDominio.isError()) {
                UtilJSF.addMessageContext(TypeNivelGravedad.ERROR,
                        valoresDominio.getCodigoError() + " : "
                                + valoresDominio.getDescripcionError());
            } else {
     ...
     * @param parametros
     */
    private ValoresDominio pingConsultaRemota(
            final List<ParametroDominio> parametros) {
        return dominioResolucionService.realizarConsultaRemota(
                this.dominio.getAmbito(), UtilJSF.getIdEntidad(),
                this.dominio.getIdentificador(), this.dominio.getUrl(),
                parametros);
    }
     ...
     */
    private ValoresDominio pingFuenteDatos(
            final List<ValorParametroDominio> parametros) {
        return dominioResolucionService.realizarConsultaFuenteDatos(
                dominio.getIdentificador(), parametros);
    }
