...
public class ViewTramites extends ViewControllerBase {
    ...
    public void init() {
        ...
        // Control acceso
        UtilJSF.verificarAccesoAdministradorDesarrolladorEntidadByEntidad(
                idEntidad);
        // Titulo pantalla
        ...
        idArea = ((HttpServletRequest) FacesContext.getCurrentInstance()
                .getExternalContext().getRequest()).getParameter("area");
        buscarAreas();
    }
    ...
    public void nuevaArea() {
        UtilJSF.openDialog(DialogArea.class, TypeModoAcceso.ALTA, null, true,
                520, 160);
    }
    ...
    public void eliminarArea() {
        ...
        if (tramiteService.removeArea(area.getCodigo())) {
            ...
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                    UtilJSF.getLiteral("info.borrado.ok"));
        } else {
            UtilJSF.addMessageContext(TypeNivelGravedad.WARNING,
                    UtilJSF.getLiteral("error.borrar.dependencias"));
        }
    ...
    public void dominioArea() {
        ...
        final Area area = listaAreasSeleccionadas.get(0);
        UtilJSF.redirectJsfPage("/secure/app/viewDominios.xhtml?ambito=A&id="
                + area.getCodigo());
    }
    ...
    public void datosArea() {
        ...
        final Area area = listaAreasSeleccionadas.get(0);
        UtilJSF.redirectJsfPage("/secure/app/viewFuentes.xhtml?ambito=A&id="
                + area.getCodigo());
    }
    ...
    public void importar() {
        ...
        UtilJSF.openDialog(DialogTramiteImportar.class, TypeModoAcceso.EDICION,
                params, true, 900, 520);
    }
    ...
    public final void returnDialogoImportar(final SelectEvent event) {
        ...
        if (!respuesta.isCanceled()
            ...
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                    UtilJSF.getLiteral("info.importar.ok"));
    ...
    public void nuevoTramite() {
        ...
        params.put(TypeParametroVentana.AREA.toString(),
                String.valueOf(area.getCodigo()));
        UtilJSF.openDialog(DialogTramite.class, TypeModoAcceso.ALTA, params,
                true, 540, 220);
    }
    ...
    public void eliminarTramite() {
        ...
        if (tramiteService.tieneTramiteVersion(
                tramiteSeleccionada.getTramite().getCodigo())) {
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                    UtilJSF.getLiteral("info.borrado.errorTramiteVersion"));
            return;
        }

        if (tramiteService
                .removeTramite(tramiteSeleccionada.getTramite().getCodigo())) {
            buscarTramites();
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                    UtilJSF.getLiteral("info.borrado.ok"));
        } else {
    ...
    public boolean getPermiteAltaArea() {
        boolean retorno;
        if (UtilJSF.getSessionBean()
                .getActiveRole() == TypeRoleAcceso.ADMIN_ENT) {
            retorno = true;
        } else if (UtilJSF.getSessionBean()
                .getActiveRole() == TypeRoleAcceso.DESAR
                && TypeEntorno.fromString(
                        UtilJSF.getEntorno()) == TypeEntorno.DESARROLLO) {
            retorno = true;
        } else {
            retorno = false;
        }
    ...
    public boolean getPermiteAltaTramite() {
        ...
        if (UtilJSF.getSessionBean().getActiveRole() == TypeRoleAcceso.DESAR
            ...
            if (areaSeleccionada != null) {
                if (areaSeleccionada.getCodigo()
                        .compareTo(idAreaCacheada) != 0) {
                    permisosCacheados = securityService
                            .getPermisosDesarrolladorEntidadByArea(
                                    areaSeleccionada.getCodigo());
                }

                res = permisosCacheados
                        .contains(TypeRolePermisos.ADMINISTRADOR_AREA)
                        || permisosCacheados
                                .contains(TypeRolePermisos.DESARROLLADOR_AREA);
    ...
    public boolean getPermiteConsultarTramite() {
        ...
        if (UtilJSF.getSessionBean().getActiveRole() == TypeRoleAcceso.DESAR) {
            if (areaSeleccionada == null) {
                ...
                if (TypeEntorno.fromString(
                        UtilJSF.getEntorno()) == TypeEntorno.DESARROLLO) {
                    return (permisosCacheados
                            .contains(TypeRolePermisos.CONSULTA));
                } else {
                    return (permisosCacheados
                            .contains(TypeRolePermisos.CONSULTA)
                            || permisosCacheados.contains(
                                    TypeRolePermisos.ADMINISTRADOR_AREA)
                            || permisosCacheados.contains(
                                    TypeRolePermisos.DESARROLLADOR_AREA));
                }
            }
        }
        return false;
    ...
    private boolean verificarFilasSeleccionadasArea() {
        ...
        if (this.listaAreasSeleccionadas == null
                || listaAreasSeleccionadas.isEmpty()) {
    ...
    private void buscarAreas() {
        final List<Area> listaTodasAreas = tramiteService.listArea(
                UtilJSF.getSessionBean().getEntidad().getCodigo(), this.filtro);
    ...
    private void buscarTramites(final String filtro) {
        ...
        for (final Area areaSeleccionada : listaAreasSeleccionadas) {
            ...
            final List<Tramite> tramites = tramiteService
                    .listTramite(areaSeleccionada.getCodigo(), this.filtro);

            // Obtenemos activa a los tramites que tengan alguna version activa
            final List<Long> idTramites = tramiteService
                    .listTramiteVersionActiva(areaSeleccionada.getCodigo());
            ...
            for (final Tramite tramite : tramites) {
                ...
                final List<TramiteVersion> listaVersiones = tramiteService
                        .listTramiteVersion(tramite.getCodigo(), null);

                if ((StringUtils.isNotEmpty(filtro)
                        && (tramite.getIdentificador().toUpperCase()
                                .contains(filtro.toUpperCase())
                                || tramite.getDescripcion().toUpperCase()
                                        .contains(filtro.toUpperCase())))
                        || StringUtils.isEmpty(filtro))
                    listaTramiteVersiones
                            .add(new TramiteVersiones(tramite, listaVersiones));
            }

        }

        Collections.sort(listaTramiteVersiones,
                (o1, o2) -> o1.getTramite().getIdentificador()
    ...
    private void abrirArea(final TypeModoAcceso modoAcceso) {
        ...
        params.put(TypeParametroVentana.ID.toString(),
                String.valueOf(areaSeleccionada.getCodigo()));
        UtilJSF.openDialog(DialogArea.class, modoAcceso, params, true, 520,
                160);
    }
    ...
    private void abrirTramite(final TypeModoAcceso modoAcceso) {
        ...
        params.put(TypeParametroVentana.ID.toString(), String
                .valueOf(this.tramiteSeleccionada.getTramite().getCodigo()));
        UtilJSF.openDialog(DialogTramite.class, modoAcceso, params, true, 540,
                220);
    }
    ...
    public void moverTramite() {
        ...
        if (checkDominioArea()) {
            UtilJSF.addMessageContext(TypeNivelGravedad.WARNING, UtilJSF
                    .getLiteral("viewTramites.error.tramiteConDominioAreas"));
            return;
        }
        ...
        params.put(TypeParametroVentana.ID.toString(), String
                .valueOf(this.tramiteSeleccionada.getTramite().getCodigo()));
        UtilJSF.openDialog(DialogMoverTramite.class, TypeModoAcceso.EDICION,
                params, true, 540, 180);
    }
    ...
    private boolean checkDominioArea() {
        final List<TramiteVersion> tramitesVersion = tramiteService
                .listTramiteVersion(
                        this.tramiteSeleccionada.getTramite().getCodigo(),
                        null);
        if (tramitesVersion != null) {
            for (final TramiteVersion tramiteVersion : tramitesVersion) {
                final List<Long> dominiosId = tramiteService
                        .getTramiteDominiosId(tramiteVersion.getCodigo());
                if (dominiosId != null) {
                    for (final Long dominioId : dominiosId) {
                        final Dominio dominio = dominioService
                                .loadDominio(dominioId);
    ...
    public void nuevaVersion() {
        ...
        params.put(TypeParametroVentana.ID.toString(),
                String.valueOf(idTramite));
        UtilJSF.openDialog(DialogTramiteVersion.class, TypeModoAcceso.ALTA,
                params, true, 400, 150);
    ...
    public void editarVersion() {
        ...
        params.put(TypeParametroVentana.ID.toString(),
                Arrays.asList(versionSeleccionada.getCodigo().toString()));
    ...
    public void consultarVersion() {
        ...
        final Map<String, List<String>> params = new HashMap<>();
        params.put(TypeParametroVentana.ID.toString(),
                Arrays.asList(String.valueOf(versionSeleccionada.getCodigo())));
        params.put(TypeParametroVentana.MODO_ACCESO.toString(),
                Arrays.asList(TypeModoAcceso.CONSULTA.name()));
        UtilJSF.redirectJsfPage("/secure/app/viewDefinicionVersion.xhtml",
                params);
    ...
    public void eliminarVersion() {
        ...
        // Eliminamos
        this.tramiteService
                .removeTramiteVersion(this.versionSeleccionada.getCodigo());
    ...
    public void bloquear() {
        ...
        if (!UtilJSF.checkEntorno(TypeEntorno.DESARROLLO)) {
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO, UtilJSF
                    .getLiteral("viewTramitesVersion.entorno.noDesarrollo"));
            return;
        }
        ...
        if (this.versionSeleccionada.getBloqueada()) {
            ...
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                    UtilJSF.getLiteral(
                            "viewTramitesVersion.bloquear.errorYaBloqueado"));
            return;
        }
        ...
        if (UtilJSF.getSessionBean().getActiveRole() == TypeRoleAcceso.DESAR) {
            ...
            if (!permisos.contains(TypeRolePermisos.DESARROLLADOR_AREA)
                    && !permisos
                            .contains(TypeRolePermisos.ADMINISTRADOR_AREA)) {
                UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                        UtilJSF.getLiteral("info.permisos.insuficientes"));
                return;
            }
        }

        tramiteService.bloquearTramiteVersion(
                this.versionSeleccionada.getCodigo(),
    ...
    public void desbloquear() {
        ...
        if (!this.versionSeleccionada.getBloqueada()) {
            ...
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                    UtilJSF.getLiteral(
                            "viewTramitesVersion.bloquear.errorYaDesbloqueado"));
            return;
        }

        if ((UtilJSF.getSessionBean().getActiveRole() == TypeRoleAcceso.DESAR
                && !this.versionSeleccionada.getDatosUsuarioBloqueo()
                        .equals(UtilJSF.getSessionBean().getUserName()))) {
            // Si no puedes editar, no puedes bloquear
            UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                    UtilJSF.getLiteral("info.permisos.insuficientes"));
            return;
        ...
        final Map<String, String> params = new HashMap<>();
        params.put(TypeParametroVentana.ID.toString(),
                this.versionSeleccionada.getCodigo().toString());

        UtilJSF.openDialog(DialogTramiteDesbloquear.class,
                TypeModoAcceso.EDICION, params, true, 500, 320);
    ...
    public void exportarVersion() {
        ...
        params.put(TypeParametroVentana.ID.toString(),
                this.versionSeleccionada.getCodigo().toString());
        UtilJSF.openDialog(DialogTramiteExportar.class, TypeModoAcceso.EDICION,
                params, true, 900, 520);
    }
    ...
    public void refrescar() {
        final String urlBase = systemService
                .obtenerPropiedadConfiguracion(Constantes.SISTRAMIT_REST_URL);
        final String usuario = systemService
                .obtenerPropiedadConfiguracion(Constantes.SISTRAMIT_REST_USER);
        final String pwd = systemService
                .obtenerPropiedadConfiguracion(Constantes.SISTRAMIT_REST_PWD);
        String identificador = null;

        for (final TramiteVersiones tramite : listaTramiteVersiones) {
            if (tramite.getTramite().getCodigo()
                    .equals(versionSeleccionada.getIdTramite())) {
        ...
        if (identificador != null) {
            final ResultadoError resultado = UtilRest.refrescar(urlBase,
                    usuario, pwd, "T", identificador + "#"
                            + versionSeleccionada.getNumeroVersion());
            if (resultado.getCodigo() == 1) {
                UtilJSF.addMessageContext(TypeNivelGravedad.INFO,
                        UtilJSF.getLiteral("info.refrescar"));
            } else {
                UtilJSF.addMessageContext(TypeNivelGravedad.ERROR,
                        UtilJSF.getLiteral("error.refrescar") + ": "
    ...
    public void duplicarVersion() {
        ...
        // Clonamos
        this.tramiteService.clonadoTramiteVersion(
                this.versionSeleccionada.getCodigo(),
                UtilJSF.getSessionBean().getUserName());
    ...
    public void historialVersion() {
        ...
        params.put(TypeParametroVentana.ID.toString(),
                this.versionSeleccionada.getCodigo().toString());
        UtilJSF.openDialog(DialogHistorialVersion.class,
                TypeModoAcceso.CONSULTA, params, true, 900, 520);
     ...
     */
    public void setTramiteSeleccionada(
            final TramiteVersiones tramiteSeleccionada) {
    ...
    public void setVersionSeleccionada(
            final TramiteVersion versionSeleccionado) {
    ...
    public void setListaTramiteVersiones(
            final List<TramiteVersiones> listaTramiteVersiones) {
    ...
    public void setListaAreasSeleccionadas(
            final List<Area> listaAreasSeleccionadas) {
