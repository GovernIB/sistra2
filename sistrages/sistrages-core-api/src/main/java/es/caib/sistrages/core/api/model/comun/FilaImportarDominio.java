...
public class FilaImportarDominio extends FilaImportar {
     ...
     */
    public FilaImportarDominio(final Dominio iDominio,
            final Dominio iDominioActual, final FuenteDatos fd,
            final byte[] fdContent, final FuenteDatos fdActual,
            final boolean permiteEditar) {
    ...
    private void rellenarDatosPorDefecto() {
        if (dominio != null) {
            ...
            case CONSULTA_BD:
                ...
                this.resultadoSQL = dominio.getSqlDecoded();
                altura = 400;
                break;
     ...
     *
     * Comprobación de permisos: - Si es global tienes que ser
     * superadministrador. - Si es de entidad, tienes que ser adm. entidad. - Si
     * es de area, tienes que ser adm.entidad o desarrollador con permiso de
     * adm./des. area.
     *
     * Pasos a realizar: <br />
     *
    ...
    private void checkDominios() {
        ...
        if (dominio != null && dominioActual != null
                && dominio.getAmbito() != dominioActual.getAmbito()) {
     ...
     * permite reemplazar (actualizando si quiere los params). En caso
     * contrario, puede mantener o reemplazar.
     *
     * Realmente sólo permite reemplazar cuando el tramite que se importa es
     * compatible con los parametros del dominio (o es compatible o se fuerza
     * para que no pierda compatibilidad y falle).
     *
     */
    private void checkDominioModoEdicion() {
        ...
        if (dominioActual == null || (dominio != null && !mismaEstructura())) {
            ...
            if (dominioActual == null) {
                ...
                this.estado = TypeImportarEstado.EXISTE;
                this.acciones = Arrays.asList(TypeImportarAccion.REEMPLAZAR,
                        TypeImportarAccion.MANTENER);
            }
            this.resultado = TypeImportarResultado.WARNING;
            this.visibleBoton = true;
            this.mismoTipo = (dominio != null && dominioActual != null
                    && dominioActual.getTipo() == dominio.getTipo());
    ...
    /**
     * Comprueba si es la misma estructura. Tienen que tener los mismos valores
     * de parámetros y, si es también de tipo FD, también la misma estructura.
     *
     * @return
     */
    private boolean mismaEstructura() {
        if (dominio.getTipo() == TypeDominio.FUENTE_DATOS
                || (dominioActual != null && dominioActual
                        .getTipo() == TypeDominio.FUENTE_DATOS)) {
            return mismosValores(dominio.getParametros(),
                    dominioActual.getParametros()) && mismosValoresFD();
        } else {
            return mismosValores(dominio.getParametros(),
                    dominioActual.getParametros());
        }
    }
    ...
    private boolean mismosValoresFD() {
        if (this.fuenteDatos == null || this.fuenteDatosActual == null) {
            ...
            if (this.fuenteDatos.getCampos().size() == this.fuenteDatosActual
                    .getCampos().size()) {
                boolean retorno = true;
                for (final FuenteDatosCampo campo : this.fuenteDatos
                        .getCampos()) {
                    for (final FuenteDatosCampo campo2 : this.fuenteDatosActual
                            .getCampos()) {
                        if (campo.getIdentificador()
                                .equals(campo2.getIdentificador())
                                && campo.isClavePrimaria() == campo2
                                        .isClavePrimaria()) {
                            break;
                        }
                    }
     ...
     * @return
     */
    private boolean mismosValores(final List<Propiedad> propiedades1,
            final List<Propiedad> propiedades2) {
        boolean iguales;
                ...
                if (propiedades1.size() == propiedades2.size()) {
                    ...
                    for (final Propiedad propiedad1 : propiedades1) {
                        ...
                        for (final Propiedad propiedad2 : propiedades2) {
                            if (mismosValores(propiedad1.getCodigo(),
                                    propiedad2.getCodigo())
                                    && mismosValores(propiedad1.getValor(),
                                            propiedad2.getValor())) {
                                encontrado = true;
                                break;
                            }
