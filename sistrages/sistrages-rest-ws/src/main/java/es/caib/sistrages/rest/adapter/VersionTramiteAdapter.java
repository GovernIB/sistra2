...
public class VersionTramiteAdapter {
	...
	public RVersionTramite convertir(final String idtramite, final TramiteVersion tv, final String idioma,
		...
		rVersionTramite.setControlAcceso(generaControlAcceso(tv)); // ver
																	// pantalla
		rVersionTramite.setIdEntidad(generaidEntidadfromTramite(tv.getIdTramite())); // buscar
																						// hacia
																						// arriba
	 ...
	 */
	private RVersionTramiteControlAcceso generaControlAcceso(final TramiteVersion tv) {
	 ...
	 */
	private List<RFormularioTramite> generaFormularios(final List<FormularioTramite> formulariosTramite,
			final String idioma) {
	 ...
	 */
	private List<RPlantillaFormulario> generarPlantillas(final List<PlantillaFormulario> plantillas,
			final String idioma) {
	 ...
	 * @return String
	 */
	private String generaFicheroPlantilla(final Long idPlantillaFormulario, final String idioma) {
	...
	private String getStringFormateador(final Long idFormateadorFormulario) {
		final FormateadorFormulario f = restApiService.getFormateadorFormulario(idFormateadorFormulario);
		if (f != null) {
	 ...
	 */
	private List<RPaginaFormulario> generarPaginas(final List<PaginaFormulario> paginas, final Long idForm,
			final String idioma) {
	 ...
	 */
	private List<RLineaComponentes> generarLineas(final List<LineaComponentesFormulario> lineas, final String idioma) {
		...
		if (componentes != null) {
			...
			for (final ComponenteFormulario c : componentes) {
				...
				case SELECTOR:
					...
					final RComponenteSelector resSL = generaComponenteSelector(cs, idioma);
					lc.add(resSL);
					break;
				case SECCION:
	 ...
	 */
	private List<RValorListaFija> generaListaFija(final List<ValorListaFija> ori, final String idioma) {
		...
		if (ori != null) {
			...
			for (final ValorListaFija v : ori) {
				...
				res.setDescripcion(AdapterUtils.generarLiteralIdioma(v.getDescripcion(), idioma));
	 ...
	 */
	private RFormularioExterno generaFormularioExterno(final GestorExternoFormularios ori) {
	 ...
	 */
	private RAnexoTramiteAyuda generaAyudaFichero(final Literal texto, final Fichero fichero, final String url,
			final String idioma) {
