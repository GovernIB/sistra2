package es.caib.sistra2.gte.core.interceptor;
...
import es.caib.sistra2.gte.core.api.exception.DatabaseException;
import es.caib.sistra2.gte.core.api.exception.ErrorNoControladoException;
import es.caib.sistra2.gte.core.api.exception.ServiceException;
import es.caib.sistra2.gte.core.api.exception.ServiceRollbackException;
import es.caib.sistra2.gte.core.api.model.comun.ConstantesNumero;
...
public final class NegocioInterceptorAspect {
   ...
   *          JointPoint
   */
  @Before("@annotation(es.caib.sistra2.gte.core.interceptor.NegocioInterceptor)")
   ...
   *          Objeto retornado
   */
  @AfterReturning(pointcut = "@annotation(es.caib.sistra2.gte.core.interceptor.NegocioInterceptor)", returning = "retVal")
   ...
   *          Exception
   */
  @AfterThrowing(pointcut = "@annotation(es.caib.sistra2.gte.core.interceptor.NegocioInterceptor)", throwing = "ex")
  ...
  private ServiceRollbackException auditaExcepcion(final JoinPoint jp, final Throwable ex) {
    ...
    if (ex instanceof DataAccessException) {
      ...
      // 2. Para excepciones no controladas que no sean excepciones de
      // servicio las encapsulamos en una excepcion de servicio
      final ErrorNoControladoException exNC = new ErrorNoControladoException(ex);
      logJP.error(exNC.getMessage(), ex);
      exNew = exNC;
